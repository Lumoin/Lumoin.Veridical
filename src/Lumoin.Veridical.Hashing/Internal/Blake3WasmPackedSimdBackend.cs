...
/// <summary>
/// WebAssembly PackedSimd backend for BLAKE3. Compresses four
/// independent chunks in parallel by laying out chunk state across
/// the four lanes of a <see cref="Vector128{T}"/>, then running the
/// standard sixteen-block BLAKE3 compression sequence lane-wise.
/// </summary>
/// <remarks>
/// <para>
...
/// <see cref="Blake3NeonBackend"/>; the body is identical because both
/// backends use the cross-platform
/// <see cref="Vector128"/>/<see cref="Vector128{T}"/> intrinsics and
/// no ISA-specific operations in the inner loop.
/// </para>
/// <para>
/// The <see cref="IsSupported"/> gate folds to <see langword="false"/>
/// at JIT time on every non-WASM host so this backend's body is
/// dead-code-eliminated on x86_64 desktops and ARM64 servers; under
/// a WASM runtime that implements the 128-bit SIMD proposal
/// (browser-wasm, wasi-wasm, anything wasmtime ≥ 0.20 hosts)
/// <see cref="PackedSimd.IsSupported"/> is true and
/// <see cref="Blake3BackendSelection.SelectBest"/> picks this backend
/// for chunk-parallel hashing.
/// </para>
/// <para>
/// WASM has no single-instruction 32-bit rotate; the implementation
/// uses the shift-or-shift composition the JIT lowers to the matching
/// PackedSimd shift + bitwise-or sequence.
/// </para>
/// </remarks>
internal static class Blake3WasmPackedSimdBackend
{
    /// <summary>Number of chunks processed per parallel batch (PackedSimd's four 32-bit lanes).</summary>
    ...
    /// <summary>Returns the WASM PackedSimd chunk-parallel many-chunks delegate.</summary>
    public static Blake3ManyChunksDelegate GetManyChunks() => CompressManyChunks;


    /// <summary>Returns the WASM PackedSimd backend bundle.</summary>
        ...
        Span<byte> chainingValuesOut)
    {
        if(!PackedSimd.IsSupported)
        {
            throw new PlatformNotSupportedException(
                "Blake3WasmPackedSimdBackend requires WebAssembly PackedSimd; check IsSupported before wiring it as a delegate.");
        }
    ...
    /// lane. The WASM SIMD proposal has no single-instruction 32-bit
    /// rotate; this is the shift-or-shift composition that lowers to
    /// two PackedSimd shifts and a bitwise-or.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
