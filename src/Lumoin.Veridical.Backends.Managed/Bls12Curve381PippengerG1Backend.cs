...
/// Jacobian core of <see cref="Bls12Curve381BigIntegerG1Reference"/>. The
...
/// </para>
/// </remarks>
internal static class Bls12Curve381PippengerG1Backend
{
    //Canonical BLS12-381 scalars are below 2^255; 256 covers the full byte width.
    ...
    {
        var cache = new System.Collections.Concurrent.ConcurrentDictionary<PointSetDigest, Bls12Curve381BigIntegerG1Reference.AffinePoint[]>();
        ...
        return (pointsConcatenated, scalarsConcatenated, count, result, curve) =>
        {
            ...
            if(!cache.TryGetValue(digest, out Bls12Curve381BigIntegerG1Reference.AffinePoint[]? points))
            {
    ...
    private static Bls12Curve381BigIntegerG1Reference.AffinePoint[] DecodePoints(ReadOnlySpan<byte> pointsConcatenated, int count)
    {
        int pointStride = WellKnownCurves.Bls12Curve381G1CompressedSizeBytes;
        var points = new Bls12Curve381BigIntegerG1Reference.AffinePoint[count];
        for(int i = 0; i < count; i++)
        {
            points[i] = Bls12Curve381BigIntegerG1Reference.Decode(pointsConcatenated.Slice(i * pointStride, pointStride));
        }
    ...
    private static void MultiScalarMultiplyOverDecoded(
        Bls12Curve381BigIntegerG1Reference.AffinePoint[] points,
        ReadOnlySpan<byte> scalarsConcatenated,
        int count,
        Span<byte> result)
    {
        ...
        var buckets = new Bls12Curve381BigIntegerG1Reference.JacobianPoint[1 << windowBits];

        Bls12Curve381BigIntegerG1Reference.JacobianPoint total = Bls12Curve381BigIntegerG1Reference.JacobianPoint.Identity;
        for(int window = windowCount - 1; window >= 0; window--)
        {
            if(window != windowCount - 1)
            {
                ...
                {
                    total = Bls12Curve381BigIntegerG1Reference.JacobianDouble(total);
                }
            }

            Array.Fill(buckets, Bls12Curve381BigIntegerG1Reference.JacobianPoint.Identity);
            for(int i = 0; i < count; i++)
            {
                ...
                {
                    buckets[digit] = Bls12Curve381BigIntegerG1Reference.JacobianAddMixed(buckets[digit], points[i]);
                }
            }
            ...
            //times.
            Bls12Curve381BigIntegerG1Reference.JacobianPoint running = Bls12Curve381BigIntegerG1Reference.JacobianPoint.Identity;
            Bls12Curve381BigIntegerG1Reference.JacobianPoint windowSum = Bls12Curve381BigIntegerG1Reference.JacobianPoint.Identity;
        ...
        Bls12Curve381BigIntegerG1Reference.Encode(Bls12Curve381BigIntegerG1Reference.JacobianToAffine(total), result);
    }
    ...
    //explicitly.
    private static Bls12Curve381BigIntegerG1Reference.JacobianPoint JacobianAdd(
        Bls12Curve381BigIntegerG1Reference.JacobianPoint p,
        Bls12Curve381BigIntegerG1Reference.JacobianPoint q)
    {
        if(p.IsIdentity)
        {
            return q;
        ...
        BigInteger z1Squared = Bls12Curve381BigIntegerG1Reference.Mod(p.Z * p.Z, prime);
        BigInteger z2Squared = Bls12Curve381BigIntegerG1Reference.Mod(q.Z * q.Z, prime);
        BigInteger u1 = Bls12Curve381BigIntegerG1Reference.Mod(p.X * z2Squared, prime);
        BigInteger u2 = Bls12Curve381BigIntegerG1Reference.Mod(q.X * z1Squared, prime);
        BigInteger s1 = Bls12Curve381BigIntegerG1Reference.Mod(p.Y * q.Z * z2Squared, prime);
        BigInteger s2 = Bls12Curve381BigIntegerG1Reference.Mod(q.Y * p.Z * z1Squared, prime);

        if(u1 == u2)
        {
            if(s1 == s2)
            {
                return Bls12Curve381BigIntegerG1Reference.JacobianDouble(p);
            }

            return Bls12Curve381BigIntegerG1Reference.JacobianPoint.Identity;
        }

        BigInteger h = Bls12Curve381BigIntegerG1Reference.Mod(u2 - u1, prime);
        BigInteger hSquared = Bls12Curve381BigIntegerG1Reference.Mod(h * h, prime);
        BigInteger hCubed = Bls12Curve381BigIntegerG1Reference.Mod(h * hSquared, prime);
        BigInteger r = Bls12Curve381BigIntegerG1Reference.Mod(s2 - s1, prime);
        BigInteger v = Bls12Curve381BigIntegerG1Reference.Mod(u1 * hSquared, prime);
        BigInteger xResult = Bls12Curve381BigIntegerG1Reference.Mod((r * r) - hCubed - (2 * v), prime);
        BigInteger yResult = Bls12Curve381BigIntegerG1Reference.Mod((r * (v - xResult)) - (s1 * hCubed), prime);
        BigInteger zResult = Bls12Curve381BigIntegerG1Reference.Mod(p.Z * q.Z * h, prime);

        return new Bls12Curve381BigIntegerG1Reference.JacobianPoint(xResult, yResult, zResult);
    }
}
