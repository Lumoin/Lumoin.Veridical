...
public static class ManagedMultilinearExtensionBackend
{
    ...
    /// <see cref="CreateFold(ScalarAddDelegate, ScalarSubtractDelegate, ScalarBatchMultiplyDelegate, BaseMemoryPool)"/>.
    /// </summary>
    /// <param name="backend">The scalar backend bundle (borrowed, not owned).</param>
    /// <param name="pool">The pool the returned delegate rents its column scratch from.</param>
    /// <exception cref="ArgumentNullException">When an argument is <see langword="null"/>.</exception>
    public static MleFoldDelegate CreateFold(ScalarArithmeticBackend backend, BaseMemoryPool pool)
    {
    ...
    /// given backend bundle — the convenience form of
    /// <see cref="CreateEvaluate(ScalarAddDelegate, ScalarSubtractDelegate, ScalarBatchMultiplyDelegate, BaseMemoryPool)"/>.
    /// </summary>
    /// <param name="backend">The scalar backend bundle (borrowed, not owned).</param>
    /// <param name="pool">The pool the returned delegate rents its fold scratch from.</param>
    /// <exception cref="ArgumentNullException">When an argument is <see langword="null"/>.</exception>
    public static MleEvaluateDelegate CreateEvaluate(ScalarArithmeticBackend backend, BaseMemoryPool pool)
    {
        ArgumentNullException.ThrowIfNull(backend);
