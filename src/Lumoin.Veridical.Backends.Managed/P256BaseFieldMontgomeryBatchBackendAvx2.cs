...
internal static class P256BaseFieldMontgomeryBatchBackendAvx2
{
    ...
    /// <summary>
    /// The limb where the specialized reduction subtracts <c>r</c> for the <c>−2⁰</c> term of
    /// <c>p = 2²⁵⁶ − 2²²⁴ + 2¹⁹² + 2⁹⁶ − 1</c>: the signed-sparse form adds <c>r·p</c> for quotient digit
    /// <c>r</c> by subtracting <c>r</c> at limbs {0, 7} and adding it at limbs {3, 6, 8} in 2³²-radix
    /// limb positions, rather than multiplying by the modulus.
    /// </summary>
    ...
    /// <summary><c>N'32 = −p⁻¹ mod 2³²</c> broadcast to every lane (value in the low 32 bits).</summary>
    private static Vector256<ulong> NPrime32Broadcast { get; } = Vector256.Create((ulong)Fp256MontgomeryParameters.NPrime32);

    /// <summary>The eight 32-bit P-256 base-field modulus limbs, each broadcast to every lane.</summary>
    ...
    /// <summary>Batched Montgomery-domain multiply using the generic <c>m·Modulus32</c> reduction; the delegate body behind <see cref="GetBatchMultiplyMontgomery"/>.</summary>
    ...
    /// <summary>Batched Montgomery-domain multiply using the P-256-specialized signed-sparse reduction; the delegate body behind <see cref="GetBatchMultiplyMontgomerySpecializedReduce"/>.</summary>
    ...
    /// <summary>Multiplies a batch of Montgomery-domain Fp256 elements pairwise, processing full quartets through the lane-interleaved kernel selected by <paramref name="useSpecializedReduce"/> and the trailing 1-3 elements through the counter-free scalar single-CIOS core.</summary>
    ...
    /// <summary>Constant-time final reduction: the candidate <c>t[0..7]</c> (with overflow limb <c>t[8]</c>) is &lt; 2p, so subtracts p once per lane iff the lane overflowed or the candidate is &gt;= p, selecting per lane via <see cref="Avx2.BlendVariable(Vector256{byte}, Vector256{byte}, Vector256{byte})"/>.</summary>
    ...
    /// </summary>
    /// <remarks>
    /// <see cref="LoadScalarLowHighLimbs"/> loads and byte-reverses each scalar's 32
    /// canonical bytes as one vector load plus a whole-buffer reversal, then
    /// zero-extends the low and high four of the eight reversed 32-bit lanes into two
    /// <see cref="Vector256{T}"/> of <see cref="ulong"/> with <c>WidenLower</c> and
    /// <c>WidenUpper</c>. Each of those two per-scalar vectors is transposed across
    /// the four scalars by <see cref="TransposeQuartetLanes"/>, once for limbs 0-3
    /// and once for limbs 4-7.
    /// </remarks>
    ...
    /// <paramref name="high"/> holds limbs 4-7, in increasing-significance
    /// <c>limb0..limb7</c> order.
    /// </summary>
    /// <remarks>
    ...
    /// eight 4-byte little-endian words, read in increasing offset order,
    /// equal the original buffer's eight 4-byte big-endian words read in
    /// decreasing offset order — exactly <c>limb0..limb7</c>. Reinterpreting
    /// those reversed bytes directly as <see cref="uint"/> lanes and
    /// zero-extending into <see cref="ulong"/> lanes relies on the
    /// little-endian lane layout of the x86-64 hosts this AVX2 backend
    /// runs on.
    /// </remarks>
    ...
    /// limb-major 32-bit-limb registers as <see cref="ElementsPerQuartet"/>
    /// scalar-major canonical encodings.
    /// </summary>
    /// <remarks>
    /// <see cref="TransposeQuartetLanes"/> inverts limb-major back to
    /// scalar-major — a matrix transpose is its own inverse, so the same
    /// method serves both directions — applied once to limbs 0-3 and once
    /// to limbs 4-7 to recover each scalar's low and high four-limb
    /// halves. <see cref="StoreScalarLowHighLimbs"/> narrows and writes
    /// each scalar's pair of halves back to canonical bytes.
    /// </remarks>
    ...
    /// its own output recovers the original four inputs; both the load
    /// and the store direction of the 32-bit-limb quartet path share this
    /// one implementation on that strength. The pairing stage zips
    /// sources <c>(0, 1)</c> and <c>(2, 3)</c>: <c>ZipLower</c> interleaves
    /// the lower half of each pair element-wise and <c>ZipUpper</c> does
    /// the same for the upper half. The assembly stage then selects the
