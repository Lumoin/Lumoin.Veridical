...
/// <summary>
/// NEON (ARM AArch64 Advanced SIMD) implementation of BN254 scalar add/subtract (and
/// their batch forms), the BN254 mirror of <see cref="Bls12Curve381NeonScalarBackend"/>.
/// Two scalars per pair (128-bit registers, two 64-bit lanes); the carry/borrow chain
/// across the four limbs advances both scalars in parallel.
/// </summary>
/// <remarks>
/// <para>
/// Requires AArch64 NEON for the lane-wise unsigned 64-bit compare
/// (<see cref="AdvSimd.Arm64.CompareGreaterThan(Vector128{ulong}, Vector128{ulong})"/>);
/// 32-bit ARM hosts fall through to the BigInteger reference via the dispatch facade.
/// The conditional reduction uses
...
/// </para>
/// </remarks>
internal static class Bn254NeonScalarBackend
{
    ...
    /// <summary>The number of independent scalars packed into one SIMD pair (one per 64-bit lane of <see cref="Vector128{T}"/>).</summary>
    ...
    /// <summary>SIMD inner loop: adds two scalars in parallel, two 64-bit lanes per <see cref="Vector128{T}"/>, one limb position per register.</summary>
    ...
    /// <summary>Lane-wise unsigned less-than via AArch64's native unsigned 64-bit compare: all-ones per lane where <paramref name="a"/> &lt; <paramref name="b"/> unsigned.</summary>
    ...
    /// <summary>
    /// Constant-time selection of one of two limb tuples based on a scalar boolean,
    /// using <see cref="AdvSimd.BitwiseSelect(Vector128{ulong}, Vector128{ulong}, Vector128{ulong})"/>
    /// so neither branch's bits enter the destination based on the condition value.
    /// </summary>
        ...
        Span<byte> destination)
    {
        ...
        for(int limbIndex = 0; limbIndex < LimbCount; limbIndex++)
        {
            //BitwiseSelect argument order: (selector, trueValue, falseValue).
    ...
    private static void EnsureSupported()
    {
        if(!AdvSimd.Arm64.IsSupported)
        {
            throw new PlatformNotSupportedException(
                "Bn254NeonScalarBackend requires AArch64 NEON; check IsSupported before wiring it as a delegate.");
        }
    }
