...
public static class SpartanProverExtensions
{
    extension(SpartanProver prover)
    {
        ...
        /// to <see cref="Prove(RelaxedR1csInstance, RelaxedR1csWitness, PolynomialCommitmentBlind, FiatShamirTranscript, FiatShamirHashDelegate, FiatShamirSqueezeDelegate, ScalarReduceDelegate, ScalarAddDelegate, ScalarSubtractDelegate, ScalarMultiplyDelegate, ScalarInvertDelegate, ScalarRandomDelegate, G1AddDelegate, G1ScalarMultiplyDelegate, G1MultiScalarMultiplyDelegate, MleEvaluateDelegate, MleFoldDelegate, SensitiveMemoryPool{byte})"/>
        ...
        private TProof ProveRelaxedCore<TProof>(
            ...
            Func<PolynomialCommitmentProvider, PolynomialCommitment, OuterSumcheckProverResult, InnerSumcheckProverResult, Scalar, PolynomialOpening, PolynomialOpening, SensitiveMemoryPool<byte>, TProof> assemble)
        {
    ...
    //throwing a clear error if the provider is not a BaseFold provider.
    ...
    private static Scalar[] ToScalarArray(System.Collections.Generic.IReadOnlyList<Scalar> source, SensitiveMemoryPool<byte> pool)
    {
