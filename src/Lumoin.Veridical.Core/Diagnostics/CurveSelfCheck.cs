...
public static class CurveSelfCheck
{
    ...
    /// <param name="pool">An optional pool from which to rent the generator buffer for the subgroup check. Defaults to <see cref="SensitiveMemoryPool{T}.Shared"/>.</param>
    ...
    public static CurveSelfCheckResult RunBls12Curve381(
        ...
        SensitiveMemoryPool<byte>? pool = null)
    {
        var witnesses = new List<CurveSelfCheckWitness>(4)
        {
            ...
            EvaluateWeierstrass(),
            EvaluateSubgroup(isInPrimeOrderSubgroup, pool ?? SensitiveMemoryPool<byte>.Shared)
        };
