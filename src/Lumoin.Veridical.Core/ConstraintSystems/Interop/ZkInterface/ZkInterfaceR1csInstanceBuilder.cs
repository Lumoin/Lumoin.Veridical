...
internal sealed class ZkInterfaceR1csInstanceBuilder: IZkInterfaceMessageSink
{
    ...
    public ZkInterfaceR1csInstanceBuilder(CurveParameterSet curve, SensitiveMemoryPool<byte> pool)
    {
    ...
    private sealed class TripleAccumulator
    {
        ...
        public R1csMatrix Build(int rowCount, int columnCount, CurveParameterSet curve, SensitiveMemoryPool<byte> pool)
        {
            if(rows.Count == 0)
            {
