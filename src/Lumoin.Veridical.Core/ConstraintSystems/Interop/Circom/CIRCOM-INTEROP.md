...
indexed by wire position." There is no standalone specification for this
layout; conformance means matching what real-world `.wtns` files actually
contain. The iden3 `snarkjs` encoder, documented at
<https://github.com/iden3/snarkjs/blob/master/src/wtns_utils.js>, is the
de-facto definition of that layout (see `ADAPTERS.md` § 7).
...
## § 5 How `.r1cs` and `.wtns` relate

Compiling a `.circom` source file yields two things: the constraint system
(`.r1cs`) and a witness *generator* for it. Running that generator against a
concrete input — a JSON object binding each input signal to a value, as small
as `{ "a": "3", "b": "11" }` for `multiplier2` — emits a `.wtns`: the dense
field-element vector satisfying the constraint system for that input.

The `.r1cs` is produced **once per circuit**. The `.wtns` is produced **once per
input**, and a different input yields a different witness against the same
`.r1cs`.

## § 6 Where Lumoin.Veridical sits

Lumoin.Veridical is a **consumer** of `.r1cs` and `.wtns`, not a producer of
them: it does not compile circuits and it does not execute a witness generator.
The Circom adapter (`CircomR1csReader`, `CircomWitnessReader`) parses the two binary
...
end, a fixture test asserts that the `.r1cs` and `.wtns` bytes compiled from a
`.circom` source parse into a well-formed instance and witness; that the
...
case — non-trivial field-specific coefficients, ~100 constraints, and a
constraints-before-header section order — which is what proves the reader is
robust to real-world section ordering rather than just the hand-crafted
minimum.
