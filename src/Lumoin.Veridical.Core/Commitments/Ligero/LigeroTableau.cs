...
public sealed class LigeroTableau: IDisposable
{
        ...
        SensitiveMemoryPool<byte> pool)
    {
        ...
        IMemoryOwner<byte> owner = pool.Rent(parameters.RowCount * rowStrideBytes);
        try
        {
            Span<byte> tableau = owner.Memory.Span[..(parameters.RowCount * rowStrideBytes)];
            tableau.Clear();

            FillBlindingRows(tableau, parameters, rowStrideBytes, scalarTag, random, add, subtract, multiply, invert, curve, pool);
            FillWitnessRows(tableau, parameters, rowStrideBytes, witnesses, scalarTag, random, add, subtract, multiply, invert, curve, pool);
            FillQuadraticRows(tableau, parameters, rowStrideBytes, witnesses, quadraticConstraints, scalarTag, random, add, subtract, multiply, invert, curve, pool);
        }
        catch
        {
        ...
        SensitiveMemoryPool<byte> pool)
    {
        ...
        EncodeRow(lowDegreeRow, block, blockEncoded, add, subtract, multiply, invert, curve, pool);
        ...
        EncodeRow(dotRow, dblock, blockEncoded, add, subtract, multiply, invert, curve, pool);
        ...
        EncodeRow(quadraticRow, dblock, blockEncoded, add, subtract, multiply, invert, curve, pool);
    }
        ...
        SensitiveMemoryPool<byte> pool)
    {
        ...
        for(int i = 0; i < parameters.WitnessRowCount; i++)
        {
            ...
            EncodeRow(row, block, blockEncoded, add, subtract, multiply, invert, curve, pool);
        }
    }
        ...
        SensitiveMemoryPool<byte> pool)
    {
        ...
        for(int i = 0; i < parameters.QuadraticTripleCount; i++)
        {
            ...
            EncodeRow(xRow, block, blockEncoded, add, subtract, multiply, invert, curve, pool);
            EncodeRow(yRow, block, blockEncoded, add, subtract, multiply, invert, curve, pool);
            EncodeRow(zRow, block, blockEncoded, add, subtract, multiply, invert, curve, pool);
        }
    }
