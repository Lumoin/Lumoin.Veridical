...
public sealed class LigeroParameters
{
    ...
    /// <summary>The Reed–Solomon evaluation domain — how node differences (and so the
    /// barycentric weights) are computed for the field this instance runs over.</summary>
    public LigeroNodeDomain NodeDomain { get; }
    ...
    /// <param name="nodeDomain">The Reed–Solomon evaluation domain; consecutive integers for prime fields (the default), the XOR domain for binary fields.</param>
    /// <exception cref="ArgumentOutOfRangeException">When a parameter is out of range.</exception>
    public LigeroParameters(int witnessCount, int quadraticConstraintCount, int inverseRate, int openedColumnCount, int block, LigeroNodeDomain nodeDomain = LigeroNodeDomain.ConsecutiveIntegers)
    {
