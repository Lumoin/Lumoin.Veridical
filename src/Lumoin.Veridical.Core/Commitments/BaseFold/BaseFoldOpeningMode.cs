...
///   <item><description><see cref="Hiding"/>: salted-Merkle leaves <c>hash(value ‖ salt)</c>, so the commitment root and every in-proof fold root are hiding, and each revealed fold pair carries its two leaf salts.</description></item>
...
public enum BaseFoldOpeningMode
{
    ...
    /// <summary>The hiding opening: salted-Merkle leaves and per-step leaf salts.</summary>
    Hiding,

    /// <summary>The zero-knowledge opening: hiding plus the lockstep CFS-2017 sumcheck mask.</summary>
    ZeroKnowledge
}
