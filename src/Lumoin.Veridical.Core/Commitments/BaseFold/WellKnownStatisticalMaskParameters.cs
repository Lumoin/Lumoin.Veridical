...
public static class WellKnownStatisticalMaskParameters
{
    ...
    /// <summary>The ℓ₂ search ceiling: 2^31 coordinates is far beyond any realisable commitment; reaching it indicates a logic error, not a configuration.</summary>
    private const int CoefficientVariableCountCeiling = 31;

    /// <summary>The mask univariates' degree for a quadratic sumcheck (BaseFold's <c>f·eq_z</c>, the Spartan inner): the per-variable degree bound the kernel's masked round format supports.</summary>
    private const int QuadraticDegree = 2;

    /// <summary>The mask univariates' degree for the Spartan outer cubic sumcheck: the per-variable degree bound the kernel's masked round format supports.</summary>
    private const int CubicDegree = 3;

    /// <summary>
    /// The Pedersen/IPA weighted opening's cleartext functional reveals of the committed vector:
    /// the precommitted filler sum <c>σ_F</c> and the IPA's final folded scalar. The round
    /// <c>L</c>/<c>R</c> points and <c>C_f</c> are blinded or DLOG-hard group elements — the
    /// Pedersen path's hiding is computational there, and the filler only launders what is revealed
    /// in the clear.
    /// </summary>
