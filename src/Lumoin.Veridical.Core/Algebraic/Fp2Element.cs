...
public sealed class Fp2Element: SensitiveMemory
{
    ...
    internal Fp2Element(IMemoryOwner<byte> owner, CurveParameterSet curve, Tag tag)
        : base(owner, WellKnownCurves.GetFp2SizeBytes(curve), tag)
    {
        Curve = curve;
    }
    ...
    /// <param name="pool">The pool to rent the backing buffer from.</param>
    /// <exception cref="ArgumentNullException">When <paramref name="pool"/> is <see langword="null"/>.</exception>
    public static Fp2Element Zero(CurveParameterSet curve, SensitiveMemoryPool<byte> pool)
    {
        ArgumentNullException.ThrowIfNull(pool);
    ...
    /// <param name="pool">The pool to rent the backing buffer from.</param>
    /// <exception cref="ArgumentNullException">When <paramref name="pool"/> is <see langword="null"/>.</exception>
    public static Fp2Element One(CurveParameterSet curve, SensitiveMemoryPool<byte> pool)
    {
        ArgumentNullException.ThrowIfNull(pool);
