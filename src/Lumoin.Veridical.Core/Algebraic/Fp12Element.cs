...
public sealed class Fp12Element: SensitiveMemory
{
    ...
    internal Fp12Element(IMemoryOwner<byte> owner, CurveParameterSet curve, Tag tag)
        : base(owner, WellKnownCurves.GetFp12SizeBytes(curve), tag)
    {
        Curve = curve;
    }
    ...
    /// <param name="pool">The pool to rent the backing buffer from.</param>
    /// <exception cref="ArgumentNullException">When <paramref name="pool"/> is <see langword="null"/>.</exception>
    public static Fp12Element Zero(CurveParameterSet curve, SensitiveMemoryPool<byte> pool)
    {
        ArgumentNullException.ThrowIfNull(pool);
    ...
    /// <param name="pool">The pool to rent the backing buffer from.</param>
    /// <exception cref="ArgumentNullException">When <paramref name="pool"/> is <see langword="null"/>.</exception>
    public static Fp12Element One(CurveParameterSet curve, SensitiveMemoryPool<byte> pool)
    {
        ArgumentNullException.ThrowIfNull(pool);
