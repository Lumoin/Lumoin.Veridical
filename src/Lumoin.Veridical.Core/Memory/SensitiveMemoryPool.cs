...
/// <summary>
/// A thread-safe memory pool designed for cryptographic operations that returns
/// memory of exactly the requested size. The pool automatically creates
/// size-specific internal sub-pools to optimise allocation patterns for
/// different buffer sizes commonly used in cryptographic operations.
/// </summary>
/// <typeparam name="T">The type of memory elements to be reserved, typically <see cref="byte"/>.</typeparam>
/// <remarks>
/// <para>
/// This memory pool is specifically designed for sensitive cryptographic
/// material where exact buffer sizes are required (no over-allocation), memory
/// is automatically cleared on disposal for security, size-specific pooling
/// optimises for common crypto buffer sizes, comprehensive metrics and tracing
/// support operational monitoring, and thread-safe operations support
/// concurrent cryptographic operations.
/// </para>
/// <para>
/// The pool maintains separate collections of slabs for each requested buffer
/// size, ensuring that buffers of different sizes never interfere with each
/// other and allowing for size-specific optimisation strategies.
/// </para>
/// <para>
/// Slab capacity is determined by a <see cref="SlabCapacityStrategy"/>
/// delegate, allowing callers to tune amortisation. The default strategy
/// (<see cref="DefaultCapacityStrategy"/>) allocates more segments for smaller
/// buffers (common key/hash sizes) and fewer for larger buffers. The ZK-tuned
/// strategy (<see cref="ZkpProverCapacityStrategy"/>) is provided for
/// zero-knowledge prover workloads where the dominant allocation sizes are
/// curve scalars (32 B), base-field elements and G1 compressed points (48 B),
/// G2 compressed points (96 B), and Gt elements (576 B); applications wire
/// it in explicitly when those sizes dominate.
/// </para>
/// <para>
/// The pool sits beneath any higher-level allocator (such as a phase-scoped
/// bump allocator) that prover hot paths may use to subdivide a single large
/// rented segment into many small intermediate spans without per-allocation
/// locking. This pool is the substrate; layered allocators are an additive
/// concern.
/// </para>
/// </remarks>
...
public class SensitiveMemoryPool<T>: MemoryPool<T>
{
    ...
    /// <summary>
    /// Lock object for synchronising access to the slabs dictionary and metrics.
    /// </summary>
    ...
    /// <summary>
    /// Controls whether distributed tracing activities are created for memory
    /// operations. Disable for high-frequency cryptographic workloads where
    ...
    /// <summary>
    /// Default initial capacity for new slabs when no allocation strategy is
    /// specified.
    /// </summary>
    ...
    /// <summary>
    /// Default strategy that allocates more segments for smaller buffers and
    /// fewer for larger ones. Tuned for the general cryptographic-material
    /// sizes that signing, key management, and hashing exercise.
    /// </summary>
    /// <param name="segmentSize">The size of each segment in elements.</param>
    /// <returns>The number of segments to allocate in the new slab.</returns>
    /// <example>
    ...
    /// <summary>
    /// Capacity strategy tuned for zero-knowledge prover workloads. The
    /// dominant allocation sizes during proof generation are curve scalars
    /// (32 bytes for BLS12-381 / BN254 / Pallas / Vesta / Grumpkin / Ed25519,
    /// 48 bytes for P-384, 66 bytes for P-521), base-field elements and G1
    /// compressed points (48 bytes for BLS12-381, 32 for BN254 and Pasta),
    /// G2 compressed points (96 bytes for BLS12-381), and Gt elements
    /// (576 bytes for BLS12-381, 384 for BN254). Multi-scalar multiplication
    /// and number-theoretic transforms iterate through millions of these
    /// per proof, so the small-size buckets are allocated much deeper than
    /// the general default to amortise slab construction across one proof.
    /// Larger buckets — proving keys, structured reference strings, large
    /// polynomial coefficient vectors — stay shallow because their reuse
    ...
    /// <example>
    /// <code>
    /// // Wire the ZK-tuned strategy at application startup.
    /// var pool = new SensitiveMemoryPool&lt;byte&gt;(
    ///     capacityStrategy: SensitiveMemoryPool&lt;byte&gt;.ZkpProverCapacityStrategy);
    /// </code>
    /// </example>
    public static int ZkpProverCapacityStrategy(int segmentSize) => segmentSize switch
    {
        <= 32 => 1024,
        <= 48 => 1024,
        <= 96 => 512,
    ...
    /// <remarks>
    /// Unlike the base <see cref="MemoryPool{T}.Shared"/>, this returns a
    /// lazily-initialised singleton so that callers who expect shared-state
    /// semantics get correct behaviour. The shared instance uses the default
    /// capacity strategy and has tracing enabled. Applications that want the
    /// ZK-tuned strategy should construct their own pool with
    /// <see cref="ZkpProverCapacityStrategy"/> rather than rely on the shared
    /// default.
    /// </remarks>
    ...
    /// <summary>
    /// Initialises a new instance with default settings.
    /// </summary>
    ...
    /// <summary>
    /// Initialises a new instance with the specified meter.
    /// </summary>
        ...
        bool tracingEnabled = true)
    {
        ...
        // Observable counters and instruments are registered against the supplied
        // meter so that consumers attach to a single meter name and receive every
        // pool's telemetry through it.
    ...
    /// <remarks>
    /// <para>
    /// This method is thread-safe and will automatically create size-specific
    /// slabs as needed. The returned memory is guaranteed to be exactly the
    /// requested size, unlike some memory pools that may return larger buffers
    /// for efficiency. Leaf algebraic types rely on this exact-size guarantee
    /// — a <c>Scalar</c> wraps a 32-byte buffer with no trailing
    /// region of stale bytes to think about.
    /// </para>
    /// <para>
    /// A single tracing activity spans the full rental lifecycle from rent to
    /// return. The activity records buffer size tags and a return event upon
    /// disposal. Tracing can be disabled via <see cref="TracingEnabled"/> for
    /// hot paths.
    /// </para>
    /// </remarks>
    ...
    public override IMemoryOwner<T> Rent(int bufferSize)
    {
        ...
        // behaviour: the lifecycle activity is the ambient context during the rental scope.
    ...
    /// <remarks>
    /// <para>
    /// Call this method periodically in long-running services to return unused
    /// memory to the operating system. Slabs that still have rented segments
    /// are left untouched.
    /// </para>
    /// <para>
    /// This operation acquires the pool lock for the duration of the trim.
    /// Avoid calling it on hot paths.
    /// </para>
    /// </remarks>
    ...
    internal void Return(ArraySegment<T> segment, Slab<T> slab)
    {
        ...
        using(LockObject.EnterScope())
        {
            // Clear the memory segment for security before returning. This is
            // the cleanup point that gives the next renter a known state without
            // requiring the pool itself to zero on every Rent.
    ...
    internal class Slab<TElement>: IDisposable
    {
        ...
        /// <summary>
        /// Initialises a new slab with the specified segment size and count.
        /// </summary>
        /// <param name="segmentSize">The size of each segment in elements.</param>
    ...
    private class ExactSizeMemoryOwner<TOwner>: IMemoryOwner<TOwner>
    {
        ...
        /// <summary>
        /// Initialises a new instance managing the given segment.
        /// </summary>
        ...
        /// The lifecycle activity is finalised with a return event and then disposed.
        /// </summary>
        /// <remarks>
