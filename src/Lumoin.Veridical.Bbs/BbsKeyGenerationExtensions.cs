...
#pragma warning disable CA2000 //Disposed by the returned BbsKeyPair.
            //The secret key is long-lived key material: request the hardened (native/locked, non-swappable)
            //tier, which degrades to the pinned tier where no native backing is wired. The public key below
            //is not secret and stays on the default managed tier.
            IMemoryOwner<byte> skOwner = pool.Rent(BbsSecretKey.SizeBytes, AllocationKind.Native);
