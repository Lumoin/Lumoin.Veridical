...
internal sealed class CompressedRoundPolynomialTests
{
    ...
    public void CompressDropsLinearTermAndKeepsOtherCoefficients()
    {
        ...
        using IMemoryOwner<byte> coefficientsOwner = BaseMemoryPool.Shared.Rent(coefficientBufferSize);
        ...
        using CompressedRoundPolynomial compressed = polynomial.Compress(BaseMemoryPool.Shared);
    ...
    public void DecompressReconstructsLinearTermFromValidClaim()
    {
        ...
        using IMemoryOwner<byte> claimOwner = BaseMemoryPool.Shared.Rent(elementSize);
        Span<byte> claimBytes = claimOwner.Memory.Span[..elementSize];
        claimBytes.Clear();
        WriteCanonical(new BigInteger(55), claimBytes);
        using Scalar claim = Scalar.FromCanonical(claimBytes, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);

        using Polynomial decompressed = compressed.Decompress(claim, Subtract, BaseMemoryPool.Shared);
    ...
    public void CompressDecompressRoundTripIsIdentityWhenClaimIsHonest()
    {
        ...
        Gen.Int[2, 5]
            ...
            .Sample((degree, coefficientsRaw) =>
            {
                ...
                using CompressedRoundPolynomial compressed = polynomial.Compress(BaseMemoryPool.Shared);
                using Polynomial recovered = compressed.Decompress(claim, Subtract, BaseMemoryPool.Shared);
    ...
    public void DecompressedPolynomialSatisfiesSumcheckClaimAtZeroAndOne()
    {
        ...
        using IMemoryOwner<byte> zeroOwner = BaseMemoryPool.Shared.Rent(elementSize);
        using IMemoryOwner<byte> oneOwner = BaseMemoryPool.Shared.Rent(elementSize);
        ...
        using Scalar atZero = recovered.Evaluate(zero, Evaluate, BaseMemoryPool.Shared);
        using Scalar atOne = recovered.Evaluate(one, Evaluate, BaseMemoryPool.Shared);

        using IMemoryOwner<byte> sumOwner = BaseMemoryPool.Shared.Rent(elementSize);
    ...
    public void FromCompressedBytesRoundTrips()
    {
        ...
        using IMemoryOwner<byte> bytesOwner = BaseMemoryPool.Shared.Rent(storedBufferSize);
        ...
        using CompressedRoundPolynomial compressed = CompressedRoundPolynomial.FromCompressedBytes(
            bytes, Degree, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
    ...
    public void CompressRejectsDegreeBelowTwo()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() =>
            _ = polynomial.Compress(BaseMemoryPool.Shared));
    }
    ...
    public void FromCompressedBytesRejectsDegreeBelowTwo()
    {
        ...
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() =>
            _ = CompressedRoundPolynomial.FromCompressedBytes(
                bytes, 1, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared));
    }
