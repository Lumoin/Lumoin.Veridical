...
internal sealed class SumcheckRoundTests
{
    ...
    public void CreateRoundtripsCompressedPolynomialAndChallengeBytes()
    {
        ...
        using IMemoryOwner<byte> compressedOwner = SensitiveMemoryPool<byte>.Shared.Rent(compressedBufferSize);
        ...
        using CompressedRoundPolynomial polynomial = CompressedRoundPolynomial.FromCompressedBytes(
            compressedBytes, Degree, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        using IMemoryOwner<byte> challengeOwner = SensitiveMemoryPool<byte>.Shared.Rent(elementSize);
        ...
        using Scalar challenge = Scalar.FromCanonical(challengeBytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void GetCompressedRoundPolynomialMaterialisesEquivalentLeafType()
    {
        ...
        using SumcheckRound round = SumcheckRound.Create(0, source, challenge, SensitiveMemoryPool<byte>.Shared);

        using CompressedRoundPolynomial materialised = round.GetCompressedRoundPolynomial(SensitiveMemoryPool<byte>.Shared);
        ...
        using Scalar materialisedChallenge = round.GetChallenge(SensitiveMemoryPool<byte>.Shared);
    ...
    public void TagCarriesAlgebraicIdentityAndDimensions()
    {
        ...
        using SumcheckRound round = SumcheckRound.Create(7, polynomial, challenge, SensitiveMemoryPool<byte>.Shared);
    ...
    public void CreateRejectsNegativeRoundIndex()
    {
        ...
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() =>
            _ = SumcheckRound.Create(-1, polynomial, challenge, SensitiveMemoryPool<byte>.Shared));
    }
    ...
    public void RoundOutlivesInnerObjectsAfterCreate()
    {
        ...
        SumcheckRound round;
        using(CompressedRoundPolynomial polynomial = CompressedRoundPolynomial.FromCompressedBytes(
            expectedCompressedBytes, Degree, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared))
        using(Scalar challenge = Scalar.FromCanonical(expectedChallengeBytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared))
        {
            round = SumcheckRound.Create(0, polynomial, challenge, SensitiveMemoryPool<byte>.Shared);
        }
    ...
    public void SumcheckClaimCarriesAllThreeFields()
    {
        ...
        using Scalar claimedSum = Scalar.FromCanonical(sumBytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void SumcheckResultVerifiedExposesFinalEvaluationAndChallenges()
    {
        ...
        Scalar finalEvaluation = Scalar.FromCanonical(finalBytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        ...
        Scalar challenge1 = Scalar.FromCanonical(c1, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        Scalar challenge2 = Scalar.FromCanonical(c2, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
