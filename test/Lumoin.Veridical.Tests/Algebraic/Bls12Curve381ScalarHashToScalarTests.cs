...
internal sealed class Bls12Curve381ScalarHashToScalarTests
{
    ...
    public void HashToScalarIsDeterministic()
    {
        ...
        {
            using Scalar a = Scalar.FromHashToScalar(message, DefaultDst, HashToScalar, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            using Scalar b = Scalar.FromHashToScalar(message, DefaultDst, HashToScalar, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void HashToScalarRespectsDomainSeparation()
    {
        ...
        {
            using Scalar a = Scalar.FromHashToScalar(message, dstA, HashToScalar, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            using Scalar b = Scalar.FromHashToScalar(message, dstB, HashToScalar, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            return !a.AsReadOnlySpan().SequenceEqual(b.AsReadOnlySpan());
        }, iter: IterationCount);
    }
    ...
    public void HashToScalarOutputIsInScalarField()
    {
        ...
        Gen.Byte.Array[0, 256].Sample(message =>
        {
            using Scalar produced = Scalar.FromHashToScalar(message, DefaultDst, HashToScalar, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
