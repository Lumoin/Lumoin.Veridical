...
internal sealed class PolynomialEvaluationTests
{
    ...
    public void HornerEvaluationMatchesDirectSumForDegreeUpToEight()
    {
        ...
        Gen.Int[0, 8]
            ...
            .Sample((degree, coefficientsRaw, pointRaw) =>
            {
                ...
                using IMemoryOwner<byte> coefficientsOwner = SensitiveMemoryPool<byte>.Shared.Rent(totalCoefficientBytes);
                ...
                using IMemoryOwner<byte> pointBytesOwner = SensitiveMemoryPool<byte>.Shared.Rent(elementSize);
                ...
                using Scalar point = Scalar.FromCanonical(pointBytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
                using Scalar hornerResult = polynomial.Evaluate(point, Evaluate, SensitiveMemoryPool<byte>.Shared);
    ...
    public void EvaluationAtZeroIsConstantTerm()
    {
        ...
        using Scalar result = polynomial.Evaluate(zero, Evaluate, SensitiveMemoryPool<byte>.Shared);
    ...
    public void EvaluationAtOneIsCoefficientSum()
    {
        ...
        using Scalar result = polynomial.Evaluate(one, Evaluate, SensitiveMemoryPool<byte>.Shared);
    ...
    public void AdditionIsCommutativeForRandomInputs()
    {
        Gen.Int[0, 8]
            ...
            .Sample((degree, aRaw, bRaw) =>
            {
                ...
                using IMemoryOwner<byte> aOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
                using IMemoryOwner<byte> bOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
                ...
                using Polynomial a = Polynomial.FromCoefficients(aCoeffs, degree, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
                using Polynomial b = Polynomial.FromCoefficients(bCoeffs, degree, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

                using Polynomial sumAb = a.Add(b, Add, SensitiveMemoryPool<byte>.Shared);
                using Polynomial sumBa = b.Add(a, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultiplicationByZeroIsZero()
    {
        ...
        using Polynomial zero = Polynomial.Zero(Degree, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using Polynomial product = p.Multiply(zero, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void StorageDegreeAfterMultiplicationEqualsSum()
    {
        ...
        using IMemoryOwner<byte> aOwner = SensitiveMemoryPool<byte>.Shared.Rent(3 * elementSize);
        ...
        using IMemoryOwner<byte> bOwner = SensitiveMemoryPool<byte>.Shared.Rent(2 * elementSize);
        ...
        using Polynomial a = Polynomial.FromCoefficients(aCoeffs, 2, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using Polynomial b = Polynomial.FromCoefficients(bCoeffs, 1, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using Polynomial product = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
