...
/// <see cref="Bls12Curve381ConstantTimeG1Backend"/> scalar-multiply delegate against the
/// variable-time BigInteger oracle <see cref="Bls12Curve381BigIntegerG1Reference"/>. Every
...
/// byte-identical 48-byte compressed output, since the constant-time ladder is a drop-in
/// replacement at the BBS signing and proof seams. Byte identity here certifies the ladder
/// arithmetic and normalization; the compressed codec is shared code between the two sides
/// and is separately pinned against published bytes by the IETF vector suites. These are
/// millisecond-scale operations, so the suite is not marked <c>[Slow]</c>.
/// </summary>
[TestClass]
internal sealed class Bls12Curve381ConstantTimeG1BackendAgreementTests
{
    private static CurveParameterSet Curve { get; } = CurveParameterSet.Bls12Curve381;
    private static BigInteger Order { get; } = Bls12Curve381BigIntegerG1Reference.ScalarFieldOrder;

    private static G1ScalarMultiplyDelegate ReferenceScalarMultiply { get; } = Bls12Curve381BigIntegerG1Reference.GetScalarMultiply();
    private static G1ScalarMultiplyDelegate ConstantTimeScalarMultiply { get; } = Bls12Curve381ConstantTimeG1Backend.GetScalarMultiply();
    private static ScalarReduceDelegate ReferenceReduce { get; } = Bls12Curve381BigIntegerScalarReference.GetReduce();

    /// <summary>
    /// Compressed BLS12-381 G1 point: the 48-byte x-coordinate with the compression, infinity, and
    /// larger-y flags in the top three bits of byte zero.
    /// </summary>
    private const int CompressedSize = WellKnownCurves.Bls12Curve381G1CompressedSizeBytes;
    ...
    /// </summary>
    private const int ScalarFillSalt = 0xB381;
    ...
    /// <summary>
    /// The delegate contract admits scalar spans of any length — the reference reads the span as an
    /// unsigned big-endian magnitude — so the ladder must stay byte-identical off the canonical
    /// 32-byte width too: zero-length (the empty magnitude), sub-width, and over-width spans with
    /// leading high bytes. The shared ladder walks whatever span it receives, making this the gate
    /// that pins value agreement across that whole admitted input space.
    /// </summary>
    [TestMethod]
    public void ScalarMultiplyAgreesWithReferenceAtNonCanonicalScalarLengths()
    {
        //Eight bytes past the canonical width exercises leading bytes above the group order; the fill
        //strides are arbitrary odd constants keeping every byte nonzero and every length distinct.
        const int MaxScalarLength = 40;
        const int FillLengthStride = 0x11;
        const int FillIndexStride = 0x2F;

        ReadOnlySpan<byte> generator = WellKnownCurves.GetG1GeneratorCompressed(Curve);

        Span<byte> scalarBuffer = stackalloc byte[MaxScalarLength];
        Span<byte> expected = stackalloc byte[CompressedSize];
        Span<byte> actual = stackalloc byte[CompressedSize];
        for(int length = 0; length <= MaxScalarLength; length++)
        {
            Span<byte> scalar = scalarBuffer[..length];
            for(int i = 0; i < length; i++)
            {
                scalar[i] = (byte)(((length * FillLengthStride) + (i * FillIndexStride)) | 1);
            }

            ReferenceScalarMultiply(generator, scalar, expected, Curve);
            ConstantTimeScalarMultiply(generator, scalar, actual, Curve);

            Assert.IsTrue(expected.SequenceEqual(actual), $"Constant-time scalar multiply diverged from the reference at scalar length {length}.");
        }
    }
