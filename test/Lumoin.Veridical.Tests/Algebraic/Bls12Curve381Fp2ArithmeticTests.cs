...
internal sealed class Bls12Curve381Fp2ArithmeticTests
{
    ...
    public void AdditionIsCommutative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes * 2]
            .Sample(raw =>
            {
                ...
                using Fp2Element ab = a.Add(b, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element ba = b.Add(a, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void AdditionIsAssociative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp2Element ab = a.Add(b, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element abPlusC = ab.Add(c, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element bc = b.Add(c, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element aPlusBc = a.Add(bc, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultiplicationIsCommutative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes * 2]
            .Sample(raw =>
            {
                using Fp2Element a = ReduceAndWrap(raw.AsSpan(0, WellKnownCurves.Bls12Curve381Fp2SizeBytes));
                using Fp2Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp2SizeBytes, WellKnownCurves.Bls12Curve381Fp2SizeBytes));
                using Fp2Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element ba = b.Multiply(a, Multiply, SensitiveMemoryPool<byte>.Shared);
                return ab.AsReadOnlySpan().SequenceEqual(ba.AsReadOnlySpan());
            }, iter: IterationCount);
    }
    ...
    public void MultiplicationIsAssociative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp2Element c = ReduceAndWrap(raw.AsSpan(2 * WellKnownCurves.Bls12Curve381Fp2SizeBytes, WellKnownCurves.Bls12Curve381Fp2SizeBytes));
                using Fp2Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element abTimesC = ab.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element bc = b.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element aTimesBc = a.Multiply(bc, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void DistributivityHolds()
    {
        ...
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp2Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp2SizeBytes, WellKnownCurves.Bls12Curve381Fp2SizeBytes));
                using Fp2Element c = ReduceAndWrap(raw.AsSpan(2 * WellKnownCurves.Bls12Curve381Fp2SizeBytes, WellKnownCurves.Bls12Curve381Fp2SizeBytes));
                using Fp2Element bPlusC = b.Add(c, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element left = a.Multiply(bPlusC, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element ac = a.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element right = ab.Add(ac, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void SquareEqualsSelfMultiply()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp2Element viaSquare = a.Square(Square, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element viaMultiply = a.Multiply(a, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void NegationCancelsAddition()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes]
            ...
            {
                using Fp2Element a = ReduceAndWrap(raw);
                using Fp2Element negA = a.Negate(Negate, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element sum = a.Add(negA, Add, SensitiveMemoryPool<byte>.Shared);
                return sum.IsZero;
            }, iter: IterationCount);
    }
    ...
    public void SubtractionEqualsAddNegate()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes * 2]
            .Sample(raw =>
            {
                using Fp2Element a = ReduceAndWrap(raw.AsSpan(0, WellKnownCurves.Bls12Curve381Fp2SizeBytes));
                using Fp2Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp2SizeBytes, WellKnownCurves.Bls12Curve381Fp2SizeBytes));
                using Fp2Element direct = a.Subtract(b, Subtract, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element negB = b.Negate(Negate, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element viaAdd = a.Add(negB, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultiplicativeInverseRoundTrips()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp2Element inverse = a.Invert(Invert, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element product = a.Multiply(inverse, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void ConjugateIsInvolutive()
    {
        ...
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes]
            ...
            {
                using Fp2Element a = ReduceAndWrap(raw);
                using Fp2Element once = a.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
                using Fp2Element twice = once.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
    ...
    public void FrobeniusEqualsConjugate()
    {
        ...
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp2Element viaConjugate = a.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        using Fp2Element zero = Fp2Element.Zero(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes]
            .Sample(raw =>
            {
                using Fp2Element a = ReduceAndWrap(raw);
                using Fp2Element sum = a.Add(zero, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        using Fp2Element one = Fp2Element.One(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp2SizeBytes]
            .Sample(raw =>
            {
                using Fp2Element a = ReduceAndWrap(raw);
                using Fp2Element product = a.Multiply(one, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void FromComponentsRoundTripsThroughAccessors()
    {
        ...
        using Fp2Element element = Fp2Element.FromComponents(c0, c1, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    private static Fp2Element ReduceAndWrap(ReadOnlySpan<byte> raw)
    {
        ...
        return Fp2Element.FromCanonical(packed, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
