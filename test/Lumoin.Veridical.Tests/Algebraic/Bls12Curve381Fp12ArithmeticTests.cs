...
internal sealed class Bls12Curve381Fp12ArithmeticTests
{
    ...
    public void AdditionIsCommutative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes * 2]
            .Sample(raw =>
            {
                ...
                using Fp12Element ab = a.Add(b, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element ba = b.Add(a, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultiplicationIsCommutative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes * 2]
            .Sample(raw =>
            {
                using Fp12Element a = ReduceAndWrap(raw.AsSpan(0, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp12SizeBytes, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element ba = b.Multiply(a, Multiply, SensitiveMemoryPool<byte>.Shared);
                return ab.AsReadOnlySpan().SequenceEqual(ba.AsReadOnlySpan());
            }, iter: IterationCount);
    }
    ...
    public void MultiplicationIsAssociative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp12Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element abTimesC = ab.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element bc = b.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element aTimesBc = a.Multiply(bc, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void DistributivityHolds()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp12Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp12SizeBytes, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element c = ReduceAndWrap(raw.AsSpan(2 * WellKnownCurves.Bls12Curve381Fp12SizeBytes, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element bPlusC = b.Add(c, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element left = a.Multiply(bPlusC, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element ac = a.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element right = ab.Add(ac, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void SquareEqualsSelfMultiply()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp12Element viaSquare = a.Square(Square, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element viaMultiply = a.Multiply(a, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void NegationCancelsAddition()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            ...
            {
                using Fp12Element a = ReduceAndWrap(raw);
                using Fp12Element negA = a.Negate(Negate, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element sum = a.Add(negA, Add, SensitiveMemoryPool<byte>.Shared);
                return sum.IsZero;
            }, iter: IterationCount);
    }
    ...
    public void SubtractionEqualsAddNegate()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes * 2]
            .Sample(raw =>
            {
                using Fp12Element a = ReduceAndWrap(raw.AsSpan(0, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp12SizeBytes, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element direct = a.Subtract(b, Subtract, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element negB = b.Negate(Negate, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element viaAdd = a.Add(negB, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultiplicativeInverseRoundTrips()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp12Element inverse = a.Invert(Invert, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element product = a.Multiply(inverse, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void ConjugateIsInvolutive()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            ...
            {
                using Fp12Element a = ReduceAndWrap(raw);
                using Fp12Element once = a.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element twice = once.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
    ...
    public void ConjugateIsAHomomorphism()
    {
        ...
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes * 2]
            .Sample(raw =>
            {
                using Fp12Element a = ReduceAndWrap(raw.AsSpan(0, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp12SizeBytes, WellKnownCurves.Bls12Curve381Fp12SizeBytes));
                using Fp12Element product = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element conjOfProduct = product.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element conjA = a.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element conjB = b.Conjugate(Conjugate, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element productOfConj = conjA.Multiply(conjB, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        using Fp12Element zero = Fp12Element.Zero(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            .Sample(raw =>
            {
                using Fp12Element a = ReduceAndWrap(raw);
                using Fp12Element sum = a.Add(zero, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        using Fp12Element one = Fp12Element.One(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            .Sample(raw =>
            {
                using Fp12Element a = ReduceAndWrap(raw);
                using Fp12Element product = a.Multiply(one, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void WSquaredEqualsV()
    {
        ...
        using Fp12Element w = Fp12Element.FromCanonical(wBytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        using Fp12Element wSquared = w.Multiply(w, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void FromComponentsRoundTripsThroughAccessors()
    {
        ...
        using Fp12Element element = Fp12Element.FromComponents(c0, c1, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    private static Fp12Element ReduceAndWrap(ReadOnlySpan<byte> raw)
    {
        ...
        return Fp12Element.FromCanonical(packed, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
