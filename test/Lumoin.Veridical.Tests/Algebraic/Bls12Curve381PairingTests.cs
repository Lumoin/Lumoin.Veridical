...
internal sealed class Bls12Curve381PairingTests
{
        ...
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            .Sample(raw =>
            {
                ...
                try
                {
                    for(int i = 0; i < 12; i++)
                    {
                        Fp12Element next = current.Frobenius(Frobenius, SensitiveMemoryPool<byte>.Shared);
                        current.Dispose();
                        current = next;
                    }
    ...
    public void FrobeniusFixesFpEmbedding()
    {
        ...
        using Fp12Element fpElement = Fp12Element.FromCanonical(bytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using Fp12Element frobenius = fpElement.Frobenius(Frobenius, SensitiveMemoryPool<byte>.Shared);
    ...
    public void CyclotomicSquareAgreesWithRegularSquare()
    {
        ...
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp12SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp12Element cycSq = a.CyclotomicSquare(CyclotomicSquare, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element regSq = a.Square(RegularSquare, SensitiveMemoryPool<byte>.Shared);
    ...
    public void PairingOfGeneratorsIsNonTrivial()
    {
        ...
        using G1Point g1 = G1Point.Generator(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using G2Point g2 = G2Point.Generator(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        using Fp12Element result = g1.PairWith(g2, Pair, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        using G1Point identity = G1Point.Identity(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using G2Point g2 = G2Point.Generator(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        using Fp12Element result = identity.PairWith(g2, Pair, SensitiveMemoryPool<byte>.Shared);
    ...
    public void PairingWithG2IdentityIsOne()
    {
        ...
        using Fp12Element result = g1.PairWith(identity, Pair, SensitiveMemoryPool<byte>.Shared);
    ...
    public void PairingIsBilinearAcrossG1AndG2()
    {
        ...
        Gen.Byte.Array[Scalar.SizeBytes]
            .Sample(raw =>
            {
                ...
                using G1Point g1 = G1Point.Generator(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
                using G2Point g2 = G2Point.Generator(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
                using G1Point aG1 = g1.ScalarMultiply(a, G1ScalarMul, SensitiveMemoryPool<byte>.Shared);
                using G2Point aG2 = g2.ScalarMultiply(a, G2ScalarMul, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element leftPairing = aG1.PairWith(g2, Pair, SensitiveMemoryPool<byte>.Shared);
                using Fp12Element rightPairing = g1.PairWith(aG2, Pair, SensitiveMemoryPool<byte>.Shared);
