...
internal sealed class Bls12Curve381ScalarHashToScalarShake256Tests
{
    ...
    public void HashToScalarIsDeterministic()
    {
        ...
        {
            using Scalar a = Scalar.FromHashToScalar(message, DefaultDst, HashToScalarShake256, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            using Scalar b = Scalar.FromHashToScalar(message, DefaultDst, HashToScalarShake256, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void HashToScalarRespectsDomainSeparation()
    {
        ...
        {
            using Scalar a = Scalar.FromHashToScalar(message, dstA, HashToScalarShake256, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            using Scalar b = Scalar.FromHashToScalar(message, dstB, HashToScalarShake256, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            return !a.AsReadOnlySpan().SequenceEqual(b.AsReadOnlySpan());
        }, iter: IterationCount);
    }
    ...
    public void HashToScalarOutputIsInScalarField()
    {
        Gen.Byte.Array[0, 256].Sample(message =>
        {
            using Scalar produced = Scalar.FromHashToScalar(message, DefaultDst, HashToScalarShake256, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            byte[] copy = produced.AsReadOnlySpan().ToArray();
            try
            {
                using Scalar reparsed = Scalar.FromCanonical(copy, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void Sha256AndShake256ProduceDifferentScalars()
    {
        ...
        using Scalar sha = Scalar.FromHashToScalar(message, dst, HashToScalarSha256, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using Scalar shake = Scalar.FromHashToScalar(message, dst, HashToScalarShake256, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
