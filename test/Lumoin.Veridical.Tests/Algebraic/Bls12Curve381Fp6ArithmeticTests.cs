...
internal sealed class Bls12Curve381Fp6ArithmeticTests
{
    ...
    public void AdditionIsCommutative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes * 2]
            .Sample(raw =>
            {
                ...
                using Fp6Element ab = a.Add(b, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element ba = b.Add(a, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void AdditionIsAssociative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp6Element ab = a.Add(b, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element abPlusC = ab.Add(c, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element bc = b.Add(c, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element aPlusBc = a.Add(bc, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultiplicationIsCommutative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes * 2]
            .Sample(raw =>
            {
                using Fp6Element a = ReduceAndWrap(raw.AsSpan(0, WellKnownCurves.Bls12Curve381Fp6SizeBytes));
                using Fp6Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp6SizeBytes, WellKnownCurves.Bls12Curve381Fp6SizeBytes));
                using Fp6Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element ba = b.Multiply(a, Multiply, SensitiveMemoryPool<byte>.Shared);
                return ab.AsReadOnlySpan().SequenceEqual(ba.AsReadOnlySpan());
            }, iter: IterationCount);
    }
    ...
    public void MultiplicationIsAssociative()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp6Element c = ReduceAndWrap(raw.AsSpan(2 * WellKnownCurves.Bls12Curve381Fp6SizeBytes, WellKnownCurves.Bls12Curve381Fp6SizeBytes));
                using Fp6Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element abTimesC = ab.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element bc = b.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element aTimesBc = a.Multiply(bc, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void DistributivityHolds()
    {
        ...
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes * 3]
            .Sample(raw =>
            {
                ...
                using Fp6Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp6SizeBytes, WellKnownCurves.Bls12Curve381Fp6SizeBytes));
                using Fp6Element c = ReduceAndWrap(raw.AsSpan(2 * WellKnownCurves.Bls12Curve381Fp6SizeBytes, WellKnownCurves.Bls12Curve381Fp6SizeBytes));
                using Fp6Element bPlusC = b.Add(c, Add, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element left = a.Multiply(bPlusC, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element ab = a.Multiply(b, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element ac = a.Multiply(c, Multiply, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element right = ab.Add(ac, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void SquareEqualsSelfMultiply()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp6Element viaSquare = a.Square(Square, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element viaMultiply = a.Multiply(a, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void NegationCancelsAddition()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes]
            ...
            {
                using Fp6Element a = ReduceAndWrap(raw);
                using Fp6Element negA = a.Negate(Negate, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element sum = a.Add(negA, Add, SensitiveMemoryPool<byte>.Shared);
                return sum.IsZero;
            }, iter: IterationCount);
    }
    ...
    public void SubtractionEqualsAddNegate()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes * 2]
            .Sample(raw =>
            {
                using Fp6Element a = ReduceAndWrap(raw.AsSpan(0, WellKnownCurves.Bls12Curve381Fp6SizeBytes));
                using Fp6Element b = ReduceAndWrap(raw.AsSpan(WellKnownCurves.Bls12Curve381Fp6SizeBytes, WellKnownCurves.Bls12Curve381Fp6SizeBytes));
                using Fp6Element direct = a.Subtract(b, Subtract, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element negB = b.Negate(Negate, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element viaAdd = a.Add(negB, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultiplicativeInverseRoundTrips()
    {
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes]
            .Sample(raw =>
            {
                ...
                using Fp6Element inverse = a.Invert(Invert, SensitiveMemoryPool<byte>.Shared);
                using Fp6Element product = a.Multiply(inverse, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        using Fp6Element zero = Fp6Element.Zero(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes]
            .Sample(raw =>
            {
                using Fp6Element a = ReduceAndWrap(raw);
                using Fp6Element sum = a.Add(zero, Add, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        using Fp6Element one = Fp6Element.One(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        Gen.Byte.Array[WellKnownCurves.Bls12Curve381Fp6SizeBytes]
            .Sample(raw =>
            {
                using Fp6Element a = ReduceAndWrap(raw);
                using Fp6Element product = a.Multiply(one, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void VCubedEqualsNonResidue()
    {
        ...
        using Fp6Element v = Fp6Element.FromCanonical(vBytes, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        using Fp6Element vSquared = v.Multiply(v, Multiply, SensitiveMemoryPool<byte>.Shared);
        using Fp6Element vCubed = vSquared.Multiply(v, Multiply, SensitiveMemoryPool<byte>.Shared);
    ...
    public void FromComponentsRoundTripsThroughAccessors()
    {
        ...
        using Fp6Element element = Fp6Element.FromComponents(c0, c1, c2, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    private static Fp6Element ReduceAndWrap(ReadOnlySpan<byte> raw)
    {
        ...
        return Fp6Element.FromCanonical(packed, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
