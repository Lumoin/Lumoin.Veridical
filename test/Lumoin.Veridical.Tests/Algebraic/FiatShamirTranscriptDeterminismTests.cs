...
internal sealed class FiatShamirTranscriptDeterminismTests
{
    ...
    public void SameInputsProduceSameChallenges()
    {
        Gen.Byte.Array[32].Sample(seed =>
        {
            ...
            using IMemoryOwner<byte> oA = SensitiveMemoryPool<byte>.Shared.Rent(SqueezeByteCount);
            using IMemoryOwner<byte> oB = SensitiveMemoryPool<byte>.Shared.Rent(SqueezeByteCount);
    ...
    public void DifferentSeedsProduceDifferentChallenges()
    {
        Gen.Select(Gen.Byte.Array[32], Gen.Byte.Array[32])
            ...
            .Sample(t =>
            {
                using FiatShamirTranscript a = FiatShamirTranscript.Initialise(DomainLabel, t.Item1, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
                using FiatShamirTranscript b = FiatShamirTranscript.Initialise(DomainLabel, t.Item2, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MultipleSqueezesProduceDistinctChallenges()
    {
        ...
        using IMemoryOwner<byte> o1 = SensitiveMemoryPool<byte>.Shared.Rent(SqueezeByteCount);
        using IMemoryOwner<byte> o2 = SensitiveMemoryPool<byte>.Shared.Rent(SqueezeByteCount);
    ...
    public void AbsorbOrderMatters()
    {
        ...
        using FiatShamirTranscript x = FiatShamirTranscript.Initialise(DomainLabel, seed, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
        x.AbsorbBytes(labelA, dataA, Hash);
        x.AbsorbBytes(labelB, dataB, Hash);

        using FiatShamirTranscript y = FiatShamirTranscript.Initialise(DomainLabel, seed, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
        y.AbsorbBytes(labelB, dataB, Hash);
        y.AbsorbBytes(labelA, dataA, Hash);

        using IMemoryOwner<byte> oX = SensitiveMemoryPool<byte>.Shared.Rent(SqueezeByteCount);
        using IMemoryOwner<byte> oY = SensitiveMemoryPool<byte>.Shared.Rent(SqueezeByteCount);
    ...
    public void SqueezeCountStartsAtZeroAndIncrementsBySqueeze()
    {
        ...
        using IMemoryOwner<byte> owner = SensitiveMemoryPool<byte>.Shared.Rent(SqueezeByteCount);
