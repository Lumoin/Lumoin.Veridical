...
internal sealed class Bls12Curve381G2PointArithmeticTests
{
    ...
    {
        using G2Point generator = G2Point.Generator(CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
    ...
    public void GeneratorEncodingRoundTrips()
    {
        ...
        using G2Point identity = G2Point.Identity(CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
        using G2Point sum = generator.Add(identity, Add, BaseMemoryPool.Shared);
    ...
    {
        using G2Point generator = G2Point.Generator(CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
        using G2Point neg = generator.Negate(Negate, BaseMemoryPool.Shared);
        using G2Point negNeg = neg.Negate(Negate, BaseMemoryPool.Shared);
    ...
    public void GeneratorPlusNegativeGeneratorIsIdentity()
    {
        using G2Point generator = G2Point.Generator(CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
        using G2Point neg = generator.Negate(Negate, BaseMemoryPool.Shared);
        using G2Point sum = generator.Add(neg, Add, BaseMemoryPool.Shared);
    ...
    public void AdditionIsCommutativeOnGeneratorMultiples()
    {
        Gen.Byte.Array[2 * Scalar.SizeBytes]
            .Sample(raw =>
            {
                ...
                using G2Point p = generator.ScalarMultiply(k1, ScalarMul, BaseMemoryPool.Shared);
                using G2Point q = generator.ScalarMultiply(k2, ScalarMul, BaseMemoryPool.Shared);
                using G2Point pq = p.Add(q, Add, BaseMemoryPool.Shared);
                using G2Point qp = q.Add(p, Add, BaseMemoryPool.Shared);
    ...
    public void ScalarMultiplyByOneIsIdentity()
    {
        ...
        oneBytes[^1] = 0x01;
        using Scalar one = Scalar.FromCanonical(oneBytes, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);

        using G2Point product = generator.ScalarMultiply(one, ScalarMul, BaseMemoryPool.Shared);
    ...
    public void ScalarMultiplyByZeroIsIdentity()
    {
        ...
        using Scalar zero = Scalar.FromCanonical(zeroBytes, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);

        using G2Point product = generator.ScalarMultiply(zero, ScalarMul, BaseMemoryPool.Shared);
    ...
    public void ScalarMultiplyByOrderIsIdentity()
    {
        ...
        using Scalar rScalar = Scalar.FromCanonical(rBytes, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
        using G2Point generator = G2Point.Generator(CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);

        using G2Point product = generator.ScalarMultiply(rScalar, ScalarMul, BaseMemoryPool.Shared);
    ...
    public void DistributivityHoldsOverGenerator()
    {
        ...
        Gen.Byte.Array[2 * Scalar.SizeBytes]
            .Sample(raw =>
            {
                ...
                using G2Point lhs = generator.ScalarMultiply(kSum, ScalarMul, BaseMemoryPool.Shared);
                using G2Point pa = generator.ScalarMultiply(ka, ScalarMul, BaseMemoryPool.Shared);
                using G2Point pb = generator.ScalarMultiply(kb, ScalarMul, BaseMemoryPool.Shared);
                using G2Point rhs = pa.Add(pb, Add, BaseMemoryPool.Shared);
    ...
    public void TamperedFlagBitRejectedByOnCurveCheck()
    {
        ...
        using G2Point tamperedPoint = G2Point.FromCanonical(tampered, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
    ...
    {
        //The canonical identity encoding is exactly 0xC0 followed by zeros. An
        //infinity-flagged encoding with an extra flag bit or trailing garbage
        //must be rejected at decode rather than aliased onto the identity:
        //aliasing would pass the on-curve and subgroup checks while evading
        //byte-compare identity detection.
        Span<byte> probe = stackalloc byte[WellKnownCurves.Bls12Curve381G2CompressedSizeBytes];
        WellKnownCurves.GetG2IdentityCompressed(CurveParameterSet.Bls12Curve381).CopyTo(probe);
        probe[0] |= 0x20;
        Assert.IsFalse(IsOnCurve(probe, CurveParameterSet.Bls12Curve381), "An infinity encoding with the y-parity flag set must be rejected.");
        Assert.IsFalse(IsInPrimeOrderSubgroup(probe, CurveParameterSet.Bls12Curve381), "The subgroup check must reject an infinity encoding with the y-parity flag set.");

        WellKnownCurves.GetG2IdentityCompressed(CurveParameterSet.Bls12Curve381).CopyTo(probe);
        probe[^1] = 0x01;
        Assert.IsFalse(IsOnCurve(probe, CurveParameterSet.Bls12Curve381), "An infinity encoding with a non-zero trailing byte must be rejected.");
        Assert.IsFalse(IsInPrimeOrderSubgroup(probe, CurveParameterSet.Bls12Curve381), "The subgroup check must reject an infinity encoding with a non-zero trailing byte.");

        Assert.IsTrue(IsOnCurve(WellKnownCurves.GetG2IdentityCompressed(CurveParameterSet.Bls12Curve381), CurveParameterSet.Bls12Curve381), "The canonical identity encoding must still decode.");
    }
