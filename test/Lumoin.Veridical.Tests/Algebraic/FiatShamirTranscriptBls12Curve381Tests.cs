...
internal sealed class FiatShamirTranscriptBls12Curve381Tests
{
    ...
    public void AbsorbScalarIsDeterministic()
    {
        Gen.Byte.Array[Scalar.SizeBytes].Sample(rawBytes =>
        {
            ...
            using Scalar scalarA = Scalar.FromCanonical(reduced, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            using Scalar scalarB = Scalar.FromCanonical(reduced, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

            ReadOnlySpan<byte> seed = stackalloc byte[8];
            using FiatShamirTranscript a = FiatShamirTranscript.Initialise(DomainLabel, seed, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
            using FiatShamirTranscript b = FiatShamirTranscript.Initialise(DomainLabel, seed, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
    ...
    public void AbsorbDifferentScalarsProducesDifferentStates()
    {
        Gen.Select(Gen.Byte.Array[Scalar.SizeBytes], Gen.Byte.Array[Scalar.SizeBytes])
            ...
            .Sample(t =>
            {
                using IMemoryOwner<byte> ra = SensitiveMemoryPool<byte>.Shared.Rent(Scalar.SizeBytes);
                using IMemoryOwner<byte> rb = SensitiveMemoryPool<byte>.Shared.Rent(Scalar.SizeBytes);
                ...
                using Scalar scalarA = Scalar.FromCanonical(reducedA, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
                using Scalar scalarB = Scalar.FromCanonical(reducedB, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void AbsorbG1PointIsDeterministic()
    {
        ...
        using G1Point generatorA = G1Point.Generator(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        using G1Point generatorB = G1Point.Generator(CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void AbsorbMultilinearExtensionRoundtrip()
    {
        ...
        using IMemoryOwner<byte> bufferOwner = SensitiveMemoryPool<byte>.Shared.Rent(evalCount * elementSize);
        ...
        using MultilinearExtension mle = MultilinearExtension.FromEvaluations(buffer, VariableCount, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void AbsorbMultilinearExtensionRejectsForeignCurve()
    {
        ...
        IMemoryOwner<byte> owner = SensitiveMemoryPool<byte>.Shared.Rent((1 << VariableCount) * elementSize);
    ...
    public void SpartanRoundShapeIsDeterministic()
    {
        ...
        using IMemoryOwner<byte> coeffOwner = SensitiveMemoryPool<byte>.Shared.Rent((Degree + 1) * elementSize);
    ...
    private static void ExecuteRound(ReadOnlySpan<byte> seed, Polynomial polynomial, Span<byte> finalState)
    {
        ...
        using Scalar evaluation = polynomial.Evaluate(challenge, PolyEvaluate, SensitiveMemoryPool<byte>.Shared);
