...
internal sealed class MultilinearExtensionEvaluationTests
{
    ...
    public void EvaluateOnHypercubeMatchesEvaluationsForOneToFourVariables()
    {
        ...
        foreach(int variableCount in (int[])[1, 2, 3, 4])
        {
            ...
            for(int hypercubeIndex = 0; hypercubeIndex < evaluationCount; hypercubeIndex++)
            {
                ...
                using Scalar result = mle.Evaluate(point.AsSpan, Evaluate, SensitiveMemoryPool<byte>.Shared);
    ...
    public void FoldThenEvaluateEqualsEvaluateForRandomInputs()
    {
        ...
        Gen.Int[1, 5]
            ...
            .Sample((n, evalBytes, challengeBytes, pointBytes) =>
            {
                ...
                using IMemoryOwner<byte> reducedEvalsOwner = SensitiveMemoryPool<byte>.Shared.Rent(evalCount * elementSize);
                using IMemoryOwner<byte> reducedPointOwner = SensitiveMemoryPool<byte>.Shared.Rent(n * elementSize);
                ...
                using Scalar resultDirect = mle.Evaluate(fullPoint.AsSpan, Evaluate, SensitiveMemoryPool<byte>.Shared);

                //Path 2: fold one variable against c, then evaluate at (p_2, ..., p_n).
                using MultilinearExtension folded = mle.Fold(challenge, Fold, SensitiveMemoryPool<byte>.Shared);
                using PointArray tail = BuildPointFromBytes(reducedPoint, n - 1, elementSize);
                using Scalar resultFolded = folded.Evaluate(tail.AsSpan, Evaluate, SensitiveMemoryPool<byte>.Shared);
    ...
    {
        const int VariableCount = 3;
        using MultilinearExtension zeroMle = MultilinearExtension.Zero(VariableCount, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        ...
        using Scalar result = zeroMle.Evaluate(point.AsSpan, Evaluate, SensitiveMemoryPool<byte>.Shared);
        BigInteger resultValue = new(result.AsReadOnlySpan(), isUnsigned: true, isBigEndian: true);
    ...
    public void FoldRejectsMleWithUnwiredCurve()
    {
        ...
        IMemoryOwner<byte> owner = SensitiveMemoryPool<byte>.Shared.Rent(bufferSize);
        ...
        Assert.ThrowsExactly<ArgumentException>(() =>
            pallasMle.Fold(challenge, Fold, SensitiveMemoryPool<byte>.Shared));
    }
    ...
    private static MultilinearExtension BuildMle(BigInteger[] evaluations, int variableCount)
    {
        ...
        using IMemoryOwner<byte> owner = SensitiveMemoryPool<byte>.Shared.Rent(evaluationCount * elementSize);
        ...
        return MultilinearExtension.FromEvaluations(bytes, variableCount, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
    ...
    private static PointArray BuildPointFromValues(BigInteger[] values)
    {
        ...
        {
            owners[i] = SensitiveMemoryPool<byte>.Shared.Rent(elementSize);
            WriteCanonical(values[i], owners[i].Memory.Span[..elementSize]);
            scalars[i] = Scalar.FromCanonical(owners[i].Memory.Span[..elementSize], CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
            owners[i].Dispose();
        }
    ...
    private static PointArray BuildPointFromBytes(ReadOnlySpan<byte> packed, int count, int elementSize)
    {
        ...
        {
            scalars[i] = Scalar.FromCanonical(packed.Slice(i * elementSize, elementSize), CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        }
    ...
    private static PointArray AssembleFullPoint(Scalar head, ReadOnlySpan<byte> tail, int tailCount, int elementSize)
    {
        ...
        scalars[0] = Scalar.FromCanonical(head.AsReadOnlySpan(), CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        for(int i = 0; i < tailCount; i++)
        {
            scalars[i + 1] = Scalar.FromCanonical(tail.Slice(i * elementSize, elementSize), CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        }
    ...
    {
        using IMemoryOwner<byte> reducedOwner = SensitiveMemoryPool<byte>.Shared.Rent(Scalar.SizeBytes);
        Span<byte> reduced = reducedOwner.Memory.Span[..Scalar.SizeBytes];
        Reduce(rawBytes, reduced, CurveParameterSet.Bls12Curve381);
        return Scalar.FromCanonical(reduced, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
