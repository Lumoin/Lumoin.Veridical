...
/// <summary>
/// Agreement gate for the AVX2 lane-parallel Fp256 batch backend
/// <see cref="P256BaseFieldMontgomeryBatchBackendAvx2"/>, the Fp256 mirror of
/// <see cref="Bn254Avx2ScalarBackendAgreementTests"/>. Gated on
/// <see cref="System.Runtime.Intrinsics.X86.Avx2"/> support.
/// </summary>
/// <remarks>
/// The load-bearing test is the belt-and-suspenders one: the batch op is a single-CIOS
/// Montgomery-domain multiply, so it must equal the scalar single-CIOS
/// <see cref="P256BaseFieldMontgomeryBackend.GetMultiplyMontgomery"/> byte-for-byte on the
/// <em>same</em> Montgomery residues — comparing against the canonical multiply would be the
/// wrong oracle (off by <c>R⁻¹</c>) and could pass a wrong kernel. The end-to-end test then
/// lifts canonical operands in, batch-multiplies, and drops the result out to confirm
/// <c>a·b mod p</c> against the BigInteger reference. <see cref="MainCount"/> is not a multiple
/// of the AVX2 quartet width, so the trailing-element fallback is exercised by every run.
/// Both the live generic <c>m·Modulus32</c> reduction
/// (<see cref="P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomery"/>) and the retained
/// P-256-specialized signed-sparse reduction
/// (<see cref="P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomerySpecializedReduce"/>)
...
/// </remarks>
[TestClass]
internal sealed class P256BaseFieldMontgomeryBatchBackendAgreementTests
{
    ...
    /// <summary>Nineteen lanes: not a multiple of the AVX2 quartet width (4), so every sample runs complete SIMD groups and a three-element tail.</summary>
    ...
    public void BatchMontgomeryMultiplyAgreesWithScalarMontgomeryMultiply()
    {
        ...
        ScalarBatchMultiplyDelegate batchGeneric = P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomery();
        ScalarBatchMultiplyDelegate batchSpecialized = P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomerySpecializedReduce();
    ...
    {
        //Sweep counts straddling the quartet width so the SIMD body, the tail, and their interaction are all
    ...
    {
        AssertRejectsOversizedBuffer(P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomery().Invoke, oversizedOperand);
    }
    ...
    {
        AssertRejectsOversizedBuffer(P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomerySpecializedReduce().Invoke, oversizedOperand);
    }
    ...
    public void BatchMontgomeryOperationsIncrementExactCounts()
    {
        ...
        try
        {
            ...
            P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomery()(left, right, result, TelemetryCount, Curve);
            ...
            CryptographicOperationCounters.Reset();
            P256BaseFieldMontgomeryBatchBackendAvx2.GetBatchMultiplyMontgomerySpecializedReduce()(left, right, result, TelemetryCount, Curve);
            Assert.AreEqual((long)TelemetryCount, CryptographicOperationCounters.GetCount(CryptographicOperationKind.ScalarBatchMultiply));
            Assert.HasCount(SingleCount, CryptographicOperationCounters.Snapshot());
        }
        finally
