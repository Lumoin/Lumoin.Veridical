...
internal sealed class Bn254ScalarBatchAgreementTests
{
        ...
        foreach(int batchSize in BatchSizesToSweep)
        {
            Gen.Select(RawScalarBytesGen.Array[batchSize], RawScalarBytesGen.Array[batchSize])
                .Sample((aBatch, bBatch) =>
                {
                    ...
                    using IMemoryOwner<byte> aBufOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
                    using IMemoryOwner<byte> bBufOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
                    using IMemoryOwner<byte> batchedResultOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
                    using IMemoryOwner<byte> perElementResultOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
        ...
        ScalarBatchAddDelegate candidateDelegate)
    {
        ...
        using IMemoryOwner<byte> aBufOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
        using IMemoryOwner<byte> bBufOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
        using IMemoryOwner<byte> referenceResultOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
        using IMemoryOwner<byte> candidateResultOwner = SensitiveMemoryPool<byte>.Shared.Rent(total);
