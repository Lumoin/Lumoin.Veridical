...
internal sealed class PolynomialInspectorTests
{
    ...
    {
        using Polynomial zero = Polynomial.Zero(0, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
    ...
    public void InspectingConstantPolynomialReportsIsConstantButNotIsZero()
    {
        ...
        using IMemoryOwner<byte> constantOwner = BaseMemoryPool.Shared.Rent(elementSize);
        Span<byte> constantBytes = constantOwner.Memory.Span[..elementSize];
        WriteCanonical(new(42), constantBytes);

        using Polynomial constant = Polynomial.Constant(constantBytes, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
    ...
    {
        int elementSize = Scalar.SizeBytes;
        using IMemoryOwner<byte> coefficientsOwner = BaseMemoryPool.Shared.Rent(2 * elementSize);
        ...
        using Polynomial linear = Polynomial.FromCoefficients(coefficients, 1, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
    ...
    public void InspectingPolynomialWithZeroLeadingCoefficientsReportsIsConstantWhenAllHighCoefficientsZero()
    {
        ...
        //degree is 3.
        int elementSize = Scalar.SizeBytes;
        using IMemoryOwner<byte> coefficientsOwner = BaseMemoryPool.Shared.Rent(4 * elementSize);
        Span<byte> coefficients = coefficientsOwner.Memory.Span[..(4 * elementSize)];
        coefficients.Clear();
        WriteCanonical(new(99), coefficients.Slice(0, elementSize));

        using Polynomial p = Polynomial.FromCoefficients(coefficients, 3, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
