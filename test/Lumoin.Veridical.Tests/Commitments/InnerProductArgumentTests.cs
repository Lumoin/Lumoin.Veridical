...
internal sealed class InnerProductArgumentTests
{
    ...
    {
        using HyraxCommitmentKey key = HyraxCommitmentKey.Derive(k, TranscriptDomain, CurveParameterSet.Bls12Curve381, HashToCurve, SensitiveMemoryPool<byte>.Shared);
        ...
        using IMemoryOwner<byte> fProverOwner = SensitiveMemoryPool<byte>.Shared.Rent(k * scalarSize);
        using IMemoryOwner<byte> rProverOwner = SensitiveMemoryPool<byte>.Shared.Rent(k * scalarSize);
        using IMemoryOwner<byte> gProverOwner = SensitiveMemoryPool<byte>.Shared.Rent(k * g1Size);
        ...
        //claimed_value = <f, R>
        using IMemoryOwner<byte> claimedOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        ...
        using IMemoryOwner<byte> blindingOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        ...
        using IMemoryOwner<byte> commitOwner = SensitiveMemoryPool<byte>.Shared.Rent(g1Size);
        ...
        using IMemoryOwner<byte> roundPairsOwner = SensitiveMemoryPool<byte>.Shared.Rent(roundCount * 2 * g1Size);
        using IMemoryOwner<byte> finalScalarOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        ...
        using FiatShamirTranscript proverTx = FiatShamirTranscript.Initialise(new FiatShamirDomainLabel(TranscriptDomain), ReadOnlySpan<byte>.Empty, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
        ...
        InnerProductArgument.Prove(
            ...
            Hash, Squeeze, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        //Verify with fresh G and R copies.
        using IMemoryOwner<byte> gVerifierOwner = SensitiveMemoryPool<byte>.Shared.Rent(k * g1Size);
        using IMemoryOwner<byte> rVerifierOwner = SensitiveMemoryPool<byte>.Shared.Rent(k * scalarSize);
        ...
        using FiatShamirTranscript verifierTx = FiatShamirTranscript.Initialise(new FiatShamirDomainLabel(TranscriptDomain), ReadOnlySpan<byte>.Empty, WellKnownHashAlgorithms.Blake3, Hash, SensitiveMemoryPool<byte>.Shared);
        ...
        Span<byte> destination)
    {
        ...
        using IMemoryOwner<byte> pointsOwner = SensitiveMemoryPool<byte>.Shared.Rent(operandCount * g1Size);
        using IMemoryOwner<byte> scalarsOwner = SensitiveMemoryPool<byte>.Shared.Rent(operandCount * scalarSize);
