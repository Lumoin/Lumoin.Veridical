...
internal sealed class HyraxBindingTests
{
    ...
    public void CannotOpenWithMismatchedMle()
    {
        ...
        var (commitment, witness) = key.CommitMultilinearExtension(mleA, fixedRandom, G1Msm, SensitiveMemoryPool<byte>.Shared);
    ...
    public void CannotOpenWithIncorrectWitnessBlindings()
    {
        ...
        using(witness)
        {
            ...
            IMemoryOwner<byte> forgedOwner = SensitiveMemoryPool<byte>.Shared.Rent(HyraxOpeningWitness.GetBufferSizeBytes(witness.RowCount));
