...
internal sealed class PedersenVectorCommitmentTests
{
    ...
    {
        using HyraxCommitmentKey key = HyraxCommitmentKey.Derive(VectorLength, WellKnownHyraxDomainLabels.CanonicalSeedV1, CurveParameterSet.Bls12Curve381, HashToCurve, BaseMemoryPool.Shared);

        using PoolScalarArray values = BuildScalars([1, 2, 3, 4]);
        using Scalar blinding = MakeScalar(99);

        using G1Point first = key.Commit(values.AsSpan, blinding, G1Msm, BaseMemoryPool.Shared);
        using G1Point second = key.Commit(values.AsSpan, blinding, G1Msm, BaseMemoryPool.Shared);
    ...
    public void DifferentBlindingsProduceDifferentCommitments()
    {
        ...
        using G1Point first = key.Commit(values.AsSpan, blinding1, G1Msm, BaseMemoryPool.Shared);
        using G1Point second = key.Commit(values.AsSpan, blinding2, G1Msm, BaseMemoryPool.Shared);
    ...
    public void HomomorphismHolds()
    {
        ...
        using Scalar rSum = rA.Add(rB, ScalarAdd, BaseMemoryPool.Shared);

        using G1Point cSum = key.Commit(sum.AsSpan, rSum, G1Msm, BaseMemoryPool.Shared);
        using G1Point cA = key.Commit(a.AsSpan, rA, G1Msm, BaseMemoryPool.Shared);
        using G1Point cB = key.Commit(b.AsSpan, rB, G1Msm, BaseMemoryPool.Shared);

        using IMemoryOwner<byte> combinedOwner = BaseMemoryPool.Shared.Rent(WellKnownCurves.Bls12Curve381G1CompressedSizeBytes);
    ...
    private static PoolScalarArray AddScalarVectors(PoolScalarArray a, PoolScalarArray b)
    {
        ...
        {
            sum[i] = a.AsSpan[i].Add(b.AsSpan[i], ScalarAdd, BaseMemoryPool.Shared);
        }
