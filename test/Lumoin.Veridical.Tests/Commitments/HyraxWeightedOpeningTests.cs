...
internal sealed class HyraxWeightedOpeningTests
{
    ...
    public void WeightedOpenVerifyRoundtrip(int variableCount)
    {
        ...
        var (commitment, witness) = key.CommitVector(vector, fixedRandom, G1Msm, SensitiveMemoryPool<byte>.Shared);
    ...
    public void VerifyWithWrongClaimedValueFails()
    {
        ...
        using(FiatShamirTranscript verifierTx = NewTranscript())
        {
            ...
            using(claimedValue)
            {
                ...
                using Scalar wrong = claimedValue.Add(one, ScalarAdd, SensitiveMemoryPool<byte>.Shared);
    ...
    public void CommitVectorWithTooFewGeneratorsThrows()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() => key.CommitVector(vector, fixedRandom, G1Msm, SensitiveMemoryPool<byte>.Shared));
    }


    private static HyraxCommitmentKey DeriveKey(int vectorLength) =>
        HyraxCommitmentKey.Derive(vectorLength, WellKnownHyraxDomainLabels.CanonicalSeedV1, CurveParameterSet.Bls12Curve381, HashToCurve, SensitiveMemoryPool<byte>.Shared);
    ...
    private static Scalar ComputeDirectInnerProduct(MultilinearExtension vector, MultilinearExtension weights)
    {
        ...
        return Scalar.FromCanonical(sum, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
    ...
    private static MultilinearExtension BuildVector(int variableCount, Func<int, int> valueAt)
    {
        ...
        using IMemoryOwner<byte> bufferOwner = SensitiveMemoryPool<byte>.Shared.Rent(evaluationCount * elementSize);
        ...
        return MultilinearExtension.FromEvaluations(buffer, variableCount, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
