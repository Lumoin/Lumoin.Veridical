...
internal sealed class MerkleTreeTests
{
    ...
    {
        using IMemoryOwner<byte> leavesOwner = SensitiveMemoryPool<byte>.Shared.Rent(leafCount * DigestSizeBytes);
        Span<byte> leaves = leavesOwner.Memory.Span[..(leafCount * DigestSizeBytes)];
        FillDistinctLeaves(leaves, leafCount);

        using MerkleTree tree = MerkleTree.Build(leaves, leafCount, Blake3TwoToOne, SensitiveMemoryPool<byte>.Shared);

        for(int leafIndex = 0; leafIndex < leafCount; leafIndex++)
        {
            using MerkleAuthenticationPath path = tree.BuildPath(leafIndex, SensitiveMemoryPool<byte>.Shared);
    ...
    public void PathDepthEqualsLogOfLeafCount()
    {
        ...
        using MerkleTree tree = MerkleTree.Build(leaves, LeafCount, Blake3TwoToOne, SensitiveMemoryPool<byte>.Shared);
        using MerkleAuthenticationPath path = tree.BuildPath(0, SensitiveMemoryPool<byte>.Shared);
    ...
    public void VerifyRejectsCorruptedLeaf()
    {
        ...
        using MerkleTree tree = MerkleTree.Build(leaves, LeafCount, Blake3TwoToOne, SensitiveMemoryPool<byte>.Shared);
        using MerkleAuthenticationPath path = tree.BuildPath(3, SensitiveMemoryPool<byte>.Shared);
    ...
    public void VerifyRejectsWrongRoot()
    {
        ...
        tamperedRoot[^1] ^= 0x01;
        using MerkleRoot wrongRoot = MerkleRoot.FromBytes(tamperedRoot, SensitiveMemoryPool<byte>.Shared);
    ...
    public void VerifyRejectsWrongLeafIndex()
    {
        ...
        using MerkleAuthenticationPath path = tree.BuildPath(2, SensitiveMemoryPool<byte>.Shared);
    ...
    public void RandomTreesAuthenticateEveryLeaf()
    {
        Gen.Int[0, MaximumLeafExponent]
            ...
            {
                using MerkleTree tree = MerkleTree.Build(leafBytes, leafCount, Blake3TwoToOne, SensitiveMemoryPool<byte>.Shared);
                for(int leafIndex = 0; leafIndex < leafCount; leafIndex++)
                {
                    using MerkleAuthenticationPath path = tree.BuildPath(leafIndex, SensitiveMemoryPool<byte>.Shared);
                    bool authenticated = path.Verify(
    ...
    {
        using IMemoryOwner<byte> leavesOwner = SensitiveMemoryPool<byte>.Shared.Rent(3 * DigestSizeBytes);
        ...
        Assert.ThrowsExactly<ArgumentException>(
            () => MerkleTree.Build(captured, 3, Blake3TwoToOne, SensitiveMemoryPool<byte>.Shared).Dispose());
    }
