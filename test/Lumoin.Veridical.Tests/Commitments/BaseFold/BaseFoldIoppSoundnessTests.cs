...
internal sealed class BaseFoldIoppSoundnessTests
{
    ...
    private static MerkleTree BuildTree(IMemoryOwner<byte> codeword, int leafCount, SensitiveMemoryPool<byte> pool, List<IDisposable> disposables)
    {
    ...
    private static MerkleRoot BuildCommitment(ReadOnlySpan<byte> codeword, int codewordElements, SensitiveMemoryPool<byte> pool)
    {
