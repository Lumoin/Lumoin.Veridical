...
internal sealed class HyraxCommitmentTests
{
    ...
    public void CommitIsDeterministicGivenFixedRandomness()
    {
        ...
        var (commitmentA, witnessA) = key.CommitMultilinearExtension(mle, fixedRandom1, G1Msm, SensitiveMemoryPool<byte>.Shared);
        var (commitmentB, witnessB) = key.CommitMultilinearExtension(mle, fixedRandom2, G1Msm, SensitiveMemoryPool<byte>.Shared);
    ...
    public void DifferentMlesProduceDifferentCommitments()
    {
        ...
        var (commitmentA, witnessA) = key.CommitMultilinearExtension(mleA, fixedRandom, G1Msm, SensitiveMemoryPool<byte>.Shared);
        var (commitmentB, witnessB) = key.CommitMultilinearExtension(mleB, fixedRandomSameSequence, G1Msm, SensitiveMemoryPool<byte>.Shared);
    ...
    public void CommitRejectsForeignCurveMle()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() => key.CommitMultilinearExtension(foreignMle, fixedRandom, G1Msm, SensitiveMemoryPool<byte>.Shared));
    }
