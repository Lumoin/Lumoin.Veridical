...
internal sealed class BatchBulletproofRangeProofTests
{
    ...
    public void BatchWithOneTamperedProofIsRejected()
    {
        ...
        try
        {
            ...
            //Corrupt the third proof's IPA tail; the other three stay untampered.
            MemoryMarshal.AsMemory(proofs[2].AsReadOnlyMemory()).Span[^1] ^= 0x01;
    ...
    private static IMemoryOwner<byte> ProveBatch(RangeProofKey key, ulong[] values, List<RangeProof> proofs, BaseMemoryPool pool)
    {
        int g1Size = WellKnownCurves.GetG1CompressedSizeBytes(Curve);
