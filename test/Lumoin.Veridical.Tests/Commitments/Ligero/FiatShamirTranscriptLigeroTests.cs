...
internal sealed class FiatShamirTranscriptLigeroTests
{
    ...
    public void DistinctLabelsSeparateOtherwiseIdenticalSqueezes()
    {
        ...
        using(FiatShamirTranscript transcript = NewTranscript())
        {
            transcript.SqueezeLigeroChallengeScalars(
                new FiatShamirOperationLabel(WellKnownLigeroTranscriptLabels.LowDegreeChallenge), Count, low, Squeeze, Hash, Reduce, CurveParameterSet.None, SensitiveMemoryPool<byte>.Shared);
        }

        using(FiatShamirTranscript transcript = NewTranscript())
        {
            transcript.SqueezeLigeroChallengeScalars(
                new FiatShamirOperationLabel(WellKnownLigeroTranscriptLabels.LinearChallenge), Count, linear, Squeeze, Hash, Reduce, CurveParameterSet.None, SensitiveMemoryPool<byte>.Shared);
        }
        ...
        Span<int> indices)
    {
        ...
        using MerkleRoot root = MerkleRoot.FromBytes(rootBytes, SensitiveMemoryPool<byte>.Shared);
        using FiatShamirTranscript transcript = NewTranscript();

        transcript.AbsorbLigeroTableauRoot(root, Hash);
        transcript.SqueezeLigeroChallengeScalars(
            new FiatShamirOperationLabel(WellKnownLigeroTranscriptLabels.LowDegreeChallenge), challengeCount, challenges, Squeeze, Hash, Reduce, CurveParameterSet.None, SensitiveMemoryPool<byte>.Shared);
