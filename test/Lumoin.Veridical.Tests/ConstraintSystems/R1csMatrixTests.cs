...
internal sealed class R1csMatrixTests
{
    ...
    public void ConstructionFromSortedTriplesIsRoundTrip()
    {
        ...
        using IMemoryOwner<byte> valuesOwner = SensitiveMemoryPool<byte>.Shared.Rent(4 * Scalar.SizeBytes);
        ...
        using R1csMatrix matrix = R1csMatrix.FromSortedTriples(rows, cols, values, 3, 3, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    ...
    public void OutOfOrderRowsAreRejected()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() =>
            _ = R1csMatrix.FromSortedTriples(rows, cols, values, 3, 1, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared));
    }
    ...
    public void OutOfOrderColumnsWithinSameRowAreRejected()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() =>
            _ = R1csMatrix.FromSortedTriples(rows, cols, values, 1, 3, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared));
    }
    ...
    public void MatrixVectorProductMatchesDirectComputation()
    {
        ...
        using IMemoryOwner<byte> valuesOwner = SensitiveMemoryPool<byte>.Shared.Rent(3 * scalarSize);
        ...
        using IMemoryOwner<byte> zOwner = SensitiveMemoryPool<byte>.Shared.Rent(2 * scalarSize);
        ...
        using IMemoryOwner<byte> resultOwner = SensitiveMemoryPool<byte>.Shared.Rent(2 * scalarSize);
        Span<byte> result = resultOwner.Memory.Span[..(2 * scalarSize)];

        matrix.MatrixVectorProduct(z, result, ScalarAdd, ScalarMul, SensitiveMemoryPool<byte>.Shared);
    ...
    public void MatrixVectorProductWithZeroVectorIsZero()
    {
        ...
        using R1csMatrix matrix = R1csMatrix.FromSortedTriples(rows, cols, values, 2, 1, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        using IMemoryOwner<byte> zOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
