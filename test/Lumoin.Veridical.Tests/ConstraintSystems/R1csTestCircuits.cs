...
internal static class R1csTestCircuits
{
    ...
    public static RawR1csInstance BuildMultiplyCircuit()
    {
        ...
        using IMemoryOwner<byte> aValuesOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        Span<byte> aValues = aValuesOwner.Memory.Span[..scalarSize];
        aValues.Clear();
        aValues[scalarSize - 1] = 0x01;

        using IMemoryOwner<byte> bValuesOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        Span<byte> bValues = bValuesOwner.Memory.Span[..scalarSize];
        bValues.Clear();
        bValues[scalarSize - 1] = 0x01;

        using IMemoryOwner<byte> cValuesOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        ...
        R1csMatrix a = R1csMatrix.FromSortedTriples(rows, aColumns, aValues, 1, 4, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        R1csMatrix b = R1csMatrix.FromSortedTriples(rows, bColumns, bValues, 1, 4, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        R1csMatrix c = R1csMatrix.FromSortedTriples(rows, cColumns, cValues, 1, 4, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
