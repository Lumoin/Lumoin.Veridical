...
internal sealed class ZkInterfaceFixtureTests
{
    ...
    private static RawR1csInstance ParseInstance(byte[] bytes, CurveParameterSet curve)
    {
        ...
        return ZkInterfaceR1csReader.Reader(pipe, WellKnownR1csFormatLabel.ZkInterface, curve, BaseMemoryPool.Shared, CancellationToken.None);
    }
    ...
    private static RawR1csWitness ParseWitness(byte[] bytes, CurveParameterSet curve)
    {
        var stream = new MemoryStream(bytes, writable: false);
        PipeReader pipe = PipeReader.Create(stream);
        return ZkInterfaceWitnessReader.Reader(pipe, WellKnownR1csFormatLabel.ZkInterface, curve, BaseMemoryPool.Shared, CancellationToken.None);
    }
