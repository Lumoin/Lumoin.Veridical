...
internal sealed class RelaxedR1csTests
{
    ...
    public void NonOneURequiresMatchingError()
    {
        ...
        using R1csSatisfaction brokenResult = instance.CheckSatisfiedBy(brokenWitness, ScalarAdd, ScalarMul, SensitiveMemoryPool<byte>.Shared);
        ...
        using R1csSatisfaction correctResult = instance.CheckSatisfiedBy(correctWitness, ScalarAdd, ScalarMul, SensitiveMemoryPool<byte>.Shared);
    ...
    public void CheckSatisfiedByUsesUInTheConstantSlot()
    {
        ...
        using R1csSatisfaction wrongResult = instance.CheckSatisfiedBy(wrongErrorWitness, ScalarAdd, ScalarMul, SensitiveMemoryPool<byte>.Shared);
    ...
    private static RelaxedR1csInstance BuildColumnZeroTouchingInstance(int uValue)
    {
        ...
        using IMemoryOwner<byte> oneOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        ...
        R1csMatrix a = R1csMatrix.FromSortedTriples(rows, aColumns, oneBytes, 1, 4, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        R1csMatrix b = R1csMatrix.FromSortedTriples(rows, bColumns, oneBytes, 1, 4, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
        R1csMatrix c = R1csMatrix.FromSortedTriples(rows, cColumns, oneBytes, 1, 4, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);

        using IMemoryOwner<byte> uOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        ...
        return RelaxedR1csInstance.Create(
            a, b, c, ReadOnlySpan<byte>.Empty, uBytes, errorCommitment, SensitiveMemoryPool<byte>.Shared);
    }
    ...
    {
        int scalarSize = Scalar.SizeBytes;
        using IMemoryOwner<byte> witnessOwner = SensitiveMemoryPool<byte>.Shared.Rent(3 * scalarSize);
        ...
        using IMemoryOwner<byte> errorOwner = SensitiveMemoryPool<byte>.Shared.Rent(scalarSize);
        ...
        return RelaxedR1csWitness.FromCanonical(witness, error, CurveParameterSet.Bls12Curve381, SensitiveMemoryPool<byte>.Shared);
    }
    ...
    private static R1csMatrix CloneMatrix(R1csMatrix source)
    {
        ...
        return R1csMatrix.FromSortedTriples(rows, cols, source.GetValuesBytes(), source.RowCount, source.ColumnCount, source.Curve, SensitiveMemoryPool<byte>.Shared);
    }
    ...
    private static PolynomialCommitment BuildDummyCommitment(int rowCount, int columnCount, int variableCount)
    {
        ...
        return PolynomialCommitment.FromBytes(
            buffer, CurveParameterSet.Bls12Curve381, CommitmentScheme.Hyrax, SensitiveMemoryPool<byte>.Shared);
    }
