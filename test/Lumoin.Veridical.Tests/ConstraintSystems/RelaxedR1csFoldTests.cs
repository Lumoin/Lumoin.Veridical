...
internal sealed class RelaxedR1csFoldTests
{
    ...
    private static Scalar[] BuildPoint(int variableCount)
    {
        ...
        for(int i = 0; i < variableCount; i++)
        {
            ...
            point[i] = Scalar.FromCanonical(buffer, CurveParameterSet.Bls12Curve381, BaseMemoryPool.Shared);
        }
    ...
    private sealed class PreparedPair: IDisposable
    {
        ...
        public static PreparedPair From(RawR1csInstance rawInstance, RawR1csWitness rawWitness)
        {
            ...
            using(rawWitness)
            {
                RelaxedR1csInstance instance = rawInstance.Prepare(BaseMemoryPool.Shared);
                RelaxedR1csWitness witness = rawWitness.Prepare(rawInstance.A.RowCount, BaseMemoryPool.Shared);
                ...
                PolynomialCommitmentBlind errorOpeningWitness = PolynomialCommitmentBlind.CreateZero(
                    errorCommitmentRowCount * Scalar.SizeBytes, CurveParameterSet.Bls12Curve381, CommitmentScheme.Hyrax, BaseMemoryPool.Shared);
