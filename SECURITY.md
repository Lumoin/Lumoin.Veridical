...
The `Native` kind is backed by an injected OS allocator that this repository does not wire, so a `Native`
rent degrades to `Pinned` here; a deployment that needs locked, non-swappable memory supplies that allocator
through the seam. The library stays allocator-agnostic: it threads the pool from the top and never names a
concrete native allocator, so which protection the key material actually receives is a property of how the
consuming deployment builds the pool.

**Residual risk (what a managed runtime does not close).** The relocation-erase gap is closed for `Pinned`
secrets: a compacting collector never moves them, so the wipe reaches the only copy. What is **not** closed
without an OS-level allocator wired: paging, since managed code cannot `mlock`/`VirtualLock`, so a host
under memory pressure may swap a secret to disk; register and stack spills, where the JIT may leave a copy
of a secret scalar in a spill slot or a caller-saved register that no clear touches; a live-process memory
dump or a debugger with process access, which sees any secret while it is legitimately in use; and
side-channel observation of the secret's *use*, which is the constant-time posture's concern above, not
this section's. A deployment that must resist a hostile host or a memory-dump adversary needs OS- or
hardware-level protection (an HSM, a `CKM_*` key that never leaves the module, encrypted swap, memory
encryption), wired through the allocation seam; the library provides the WSCA-side mathematics such a
deployment assumes, not the host hardening itself.
