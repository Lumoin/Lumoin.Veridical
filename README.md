...
| **Lumoin.Veridical.Cli** | Command-line and MCP tool: platform/backend info, BLAKE3 hashing, predicate prove/verify, conformance self-tests; ships as native-AOT packages per platform | [![NuGet](https://img.shields.io/nuget/v/Lumoin.Veridical.Cli.svg?style=flat)](https://www.nuget.org/packages/Lumoin.Veridical.Cli/) |
| **Lumoin.Veridical.Longfellow** | Longfellow-style zero-knowledge statements over ECDSA-signed credentials: mdoc and JWT (JWS compact serialization) facades over the dual-field prover and verifier | in-repo, not yet published |
...
- **Polynomial commitments** — Hyrax (discrete-log), BaseFold (hash-based, with a zero-knowledge variant validated by the leakage toolkit), Ligero (interleaved Reed–Solomon), and WHIR (Reed–Solomon proximity testing with round-by-round rate improvement, a computed round-by-round soundness ledger, and a hiding variant with a witness-free transcript simulator), all behind a uniform `Commit`/`Open`/`Verify` surface so Spartan drops in any of them unchanged.
- **Lookup arguments** — LogUp set-membership claims (logarithmic-derivative multiplicity and helper columns), commitment-scheme-generic and wired to the predicate CLI/MCP surface for `memberOf` claims at a union-bounded query budget.
- **Transparent Ligero argument** — field-generic and NTT-free over a correctness-first barycentric Reed–Solomon encoder, so it runs over fields without smooth-order roots of unity, including the P-256 scalar field. This is the substrate for Longfellow-style zero-knowledge over ECDSA-signed mdoc and JWT credentials.
...
- **WHIR** (Arnon, Chiesa, Fenzi, Yogev, 2024) — the Reed–Solomon proximity-testing polynomial commitment with round-by-round rate improvement.
- **Zero-Knowledge IOPPs for Constrained Interleaved Codes** (Chiesa, Fenzi, Weissenberg, 2026) — the hiding WHIR construction: masked sumcheck, zero-knowledge code switching, and the masked base case.
